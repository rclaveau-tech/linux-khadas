...
#error "No support reserved-memory currently when the device-tree enabled."
	...
	mem_pool = adlak_os_zalloc(sizeof(struct adlak_mem_pool_info),
				   ADLAK_GFP_KERNEL);
	if (ADLAK_IS_ERR_OR_NULL(mem_pool))
		goto err_alloc;
	...
	*ptr = mem_pool;
	AML_LOG_INFO("Reserved memory info: dma_addr= 0x%lX,  phys_addr= 0x%lX,size=%lu MByte\n",
		     (uintptr_t)mem_pool->dma_addr_base,
		     (uintptr_t)mem_pool->phys_addr_base,
...
static int adlak_dma_map_of_discontiguous(struct adlak_mem *mm,
					  ...
					  u32 offset, u32 size)
{
	...
	s32 result = 0;
	u32 flush_size = ADLAK_ALIGN(size, cache_line_size());
	...
	if (size & (cache_line_size() - 1)) {
		AML_LOG_ERR
			("flush cache warning memory size 0x%x not align to cache line size 0x%x",
			size, cache_line_size());
	}

	mm_info->sgt =
		adlak_os_malloc(sizeof(struct sg_table), ADLAK_GFP_KERNEL);
	if (!mm_info->sgt) {
	...
	result = sg_alloc_table_from_pages(sgt, (struct page **)mm_info->pages,
					   mm_info->nr_pages, offset,
					   flush_size,
					   ADLAK_GFP_KERNEL | __GFP_NOWARN);
...
static int adlak_dma_map_of_contiguous(struct adlak_mem *mm,
				       struct adlak_mem_handle *mm_info,
				       u32 offset, u32 size,
				       struct page *page_continue)
{
	switch (mm_info->req.mem_direction) {
	case ADLAK_ENUM_MEM_DIR_WRITE_ONLY:
		mm_info->direction = DMA_TO_DEVICE;
	...
	mm_info->dma_addr = dma_map_page(mm->dev, page_continue, offset, size,
					 mm_info->direction);
...
static void adlak_dma_unmap_of_contiguous(struct adlak_mem *mm,
					  struct adlak_mem_handle *mm_info)
{
	if (mm_info->dma_addr) {
		dma_unmap_page(mm->dev, mm_info->dma_addr, mm_info->req.bytes,
			       mm_info->direction);
		mm_info->dma_addr = 0;
	}
}

void adlak_os_free_discontiguous(struct adlak_mem *mm,
				 struct adlak_mem_handle *mm_info)
{
	if (mm_info->pages) {
		adlak_os_mm_vunmap(mm_info);
		adlak_dma_unmap_of_discontiguous(mm, mm_info);
		adlak_os_free_pages((struct page **)mm_info->pages,
				    mm_info->nr_pages);
...
int adlak_os_alloc_discontiguous(struct adlak_mem *mm,
				 struct adlak_mem_handle *mm_info)
{
	...
	phys_addrs = (phys_addr_t *)adlak_os_zalloc(sizeof(phys_addr_t) * mm_info->nr_pages,
						    ADLAK_GFP_KERNEL);
...
void adlak_os_free_contiguous(struct adlak_mem *mm,
			      struct adlak_mem_handle *mm_info)
{
	if (mm_info->pages) {
		adlak_os_mm_vunmap(mm_info);
		adlak_dma_unmap_of_contiguous(mm, mm_info);
		adlak_os_free_pages_contiguous((struct page **)mm_info->pages,
					       mm_info->nr_pages);
		adlak_os_free(mm_info->phys_addrs);
		adlak_os_free(mm_info->pages);
		mm_info->pages = NULL;
...
int adlak_os_alloc_contiguous(struct adlak_mem *mm,
			      struct adlak_mem_handle *mm_info)
{
	struct page **pages = NULL;
	...
	if (order >= MAX_PAGE_ORDER) {
		AML_LOG_WARN
			("contiguous alloc contiguous memory order is bigger than MAX, %d >= %d\n",
	...
	for (i = 0; i < mm_info->nr_pages; ++i) {
		phys_addrs[i] = phys_addrs[0] +
				i * ADLAK_PAGE_SIZE; // get physical addr
...
int adlak_os_attach_ext_mem(struct adlak_mem *mm,
			    struct adlak_mem_handle *mm_info,
			    u64 phys_addr)
{
	phys_addr_t *phys_addrs = NULL;
	...
	for (i = 0; i < mm_info->nr_pages; ++i) {
		phys_addrs[i] =
			phys_addr + (i * ADLAK_PAGE_SIZE); // get physical addr
		AML_LOG_DEBUG("phys_addrs[%d]=0x%llx", i,
			      (u64)(uintptr_t)phys_addrs[i]);
	}
	mm_info->pages = NULL;
	mm_info->phys_addrs = phys_addrs;
	...
	if (adlak_dma_map_of_contiguous(mm, mm_info, 0, mm_info->req.bytes,
					page_continue)) {
		adlak_os_free(phys_addrs);
	}
...
void adlak_os_dettach_ext_mem(struct adlak_mem *mm,
			      struct adlak_mem_handle *mm_info)
{
...
int adlak_os_mmap(struct adlak_mem *mm, struct adlak_mem_handle *mm_info,
		  void *const _vma)
{
	unsigned long pfn;
	int i;
	...
	mm_info->mem_type = mm_info->mem_type |
			    ADLAK_ENUM_MEMTYPE_INNER_USER_CACHEABLE;
	...
	if (mm_info->mem_src == ADLAK_ENUM_MEMSRC_OS) {
		if (mm_info->mem_type & ADLAK_ENUM_MEMTYPE_INNER_CONTIGUOUS) {
			...
			remap_pfn_range(vma, vma->vm_start, pfn,
					...
					vma->vm_page_prot);
			AML_LOG_DEBUG("%s contiguous  phys_addr = 0x%lX",
				      __func__, (uintptr_t)mm_info->phys_addr);
		} else {
			AML_LOG_DEBUG("%s discontiguous phys_addr = 0x%lX",
				      __func__, (uintptr_t)mm_info->phys_addr);
			pages = (struct page **)(mm_info->pages);
			for (i = 0; i < mm_info->nr_pages; ++i) {
				pfn = page_to_pfn(pages[i]);
				if (remap_pfn_range(vma, vma->vm_start + i * ADLAK_PAGE_SIZE,
						    pfn, ADLAK_PAGE_SIZE,
						    vma->vm_page_prot)) {
					return ERR(EAGAIN);
				}
		...
		pfn = page_to_pfn(phys_to_page(mm_info->phys_addr));
		remap_pfn_range(vma, vma->vm_start, pfn, mm_info->req.bytes,
				vma->vm_page_prot);
		...
		AML_LOG_DEBUG("%s ext phys buffer,  phys_addr = 0x%lX",
			      __func__, (uintptr_t)mm_info->phys_addr);
	} else {
...
static void adlak_dma_sync_sg_partial(struct device *dev,
...
void adlak_os_flush_cache(struct adlak_mem *mm,
			  struct adlak_mem_handle *mm_info,
			  struct adlak_sync_cache_ext_info *sync_cache_ext_info)
{
	...
	if (!(mm_info->mem_type &
	      (ADLAK_ENUM_MEMTYPE_INNER_USER_CACHEABLE |
	       ADLAK_ENUM_MEMTYPE_INNER_KERNEL_CACHEABLE))) {
		return;
	}

	AML_LOG_DEBUG("%s mem_type %lX\tcpu_addr %lX\t"
		"cpu_addr_user %lX\nis_partial %d\toffset %lX\tsize %lX\n",
		__func__, (uintptr_t)mm_info->mem_type,
		(uintptr_t)mm_info->cpu_addr, (uintptr_t)mm_info->cpu_addr_user,
		(s32)sync_cache_ext_info->is_partial,
		(uintptr_t)sync_cache_ext_info->offset,
		(uintptr_t)sync_cache_ext_info->size);
	if (ADLAK_ENUM_MEMTYPE_INNER_CONTIGUOUS & mm_info->mem_type) {
		ASSERT(mm_info->dma_addr);
		if (!sync_cache_ext_info->is_partial) {
			dma_sync_single_for_device(mm->dev, mm_info->dma_addr,
						   mm_info->req.bytes,
						   DMA_TO_DEVICE);
		} else {
			dma_sync_single_for_device(mm->dev,
						   mm_info->dma_addr
						   + sync_cache_ext_info->offset,
		...
		sgt = (struct sg_table *)mm_info->sgt;
		if (!sync_cache_ext_info->is_partial) {
			dma_sync_sg_for_device(mm->dev, sgt->sgl, sgt->nents,
					       DMA_TO_DEVICE);
		} else {
			adlak_dma_sync_sg_partial(mm->dev, sgt->sgl, sgt->nents,
						  sync_cache_ext_info->offset,
						  sync_cache_ext_info->size,
						  DMA_TO_DEVICE);
		}
	}
}
			    ...
			    struct adlak_sync_cache_ext_info *sync_cache_ext_info)
{
	...
	AML_LOG_DEBUG("%s mem_type %lX\tcpu_addr %lX\t"
		      ...
		      __func__, (uintptr_t)mm_info->mem_type,
		      (uintptr_t)mm_info->cpu_addr, (uintptr_t)mm_info->cpu_addr_user,
		      sync_cache_ext_info->is_partial,
		      (uintptr_t)sync_cache_ext_info->offset,
		      (uintptr_t)sync_cache_ext_info->size);
	if (ADLAK_ENUM_MEMTYPE_INNER_CONTIGUOUS & mm_info->mem_type) {
		ASSERT(mm_info->dma_addr);
		if (!sync_cache_ext_info->is_partial) {
			dma_sync_single_for_cpu(mm->dev, mm_info->dma_addr,
						mm_info->req.bytes,
						DMA_FROM_DEVICE);
		} else {
		...
		sgt = (struct sg_table *)mm_info->sgt;
		if (!sync_cache_ext_info->is_partial) {
			dma_sync_sg_for_cpu(mm->dev, sgt->sgl, sgt->nents,
					    DMA_FROM_DEVICE);
		} else {
			adlak_dma_sync_sg_partial(mm->dev, sgt->sgl, sgt->nents,
						  sync_cache_ext_info->offset,
...
void adlak_free_through_dma(struct adlak_mem *mm,
			    struct adlak_mem_handle *mm_info)
{
	if (mm_info->cpu_addr) {
		dma_free_coherent(mm->dev, mm_info->req.bytes,
				  mm_info->cpu_addr, mm_info->dma_addr);
		mm_info->cpu_addr = NULL;
	}
}

int adlak_malloc_through_dma(struct adlak_mem *mm,
			     struct adlak_mem_handle *mm_info)
{
	gfp_t gfp = 0;
	...
	AML_LOG_DEBUG("%s: dma_addr= 0x%lX,  phys_addr= 0x%lX,size=%lu KByte\n",
		      __func__, (uintptr_t)mm_info->dma_addr,
		      (uintptr_t)mm_info->phys_addr,
		      (uintptr_t)(mm_info->req.bytes / 1024));
	return ERR(NONE);
}

int adlak_os_mmap2userspace(struct adlak_mem *mm,
			    struct adlak_mem_handle *mm_info)
{
	int ret = ERR(EINVAL);
	...
	if (ADLAK_IS_ERR_OR_NULL(mm_info))
		goto exit;
	addr_userspace = vm_mmap(NULL, 0L, mm_info->req.bytes,
...
void adlak_os_unmmap_userspace(struct adlak_mem *mm,
			       struct adlak_mem_handle *mm_info)
{
	unsigned long addr_userspace = 0;
	...
	AML_LOG_DEBUG("%s cpu_addr_user = 0x%lX", __func__,
		      (uintptr_t)mm_info->cpu_addr_user);
...
void *adlak_os_mm_vmap(struct adlak_mem_handle *mm_info)
{
	...
	/* make a long duration mapping of multiple physical pages
	 * into a contiguous virtual space
	 */

	if (!(mm_info->req.mem_type &
	      ADLAK_ENUM_MEMTYPE_INNER_KERNEL_CACHEABLE)) {
		cpu_addr = vmap(pages, mm_info->nr_pages, VM_MAP,
	...
	AML_LOG_DEBUG("%s: VA_kernel=0x%lX", __func__, (uintptr_t)cpu_addr);
