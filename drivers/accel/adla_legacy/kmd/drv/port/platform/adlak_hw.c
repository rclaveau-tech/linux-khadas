...
static int adlak_parser_set_wpt(struct io_region *region, u32 wpt)
{
	//int ret = 0;
	if (wpt % 16 != 0) {
...
void adlak_pm_enable(void *padlak, u32 en)
{
	...
	union HAL_ADLAK_PM_EN_S d;
	union HAL_ADLAK_REV_S d_rev;
...
void adlak_pm_config(void *padlak, u32 addr, u32 buf_size,
		     u32 wpt)
{
	struct io_region *region = NULL;
...
static int adlak_hal_soft_reset(void *data)
{
	...
	struct io_region *region = NULL;
	union HAL_ADLAK_AB_CTL_S d_ab;
	union HAL_ADLAK_SWRST_S d;
	u32 cnt;
	...
	while (1) {
		...
		if (d_ab.bitc.ab_force_stop_idle == 1)
			break;
...
int adlak_hal_set_mmu(void *data, bool enable, u64 smmu_entry)
{
	...
	if (enable) {
		AML_LOG_INFO("SMMU Enable, and set smmu_entry addr=0x%lX",
			     (uintptr_t)smmu_entry);
		smmu_entry = ((smmu_entry >> 5) << 12);

		adlak_write32(region, REG_ADLAK_0XC4,
			      (u32)(smmu_entry & (0xFFFFFFFF)));
		adlak_write32(region, REG_ADLAK_0XC8,
			      (u32)((smmu_entry >> 32) & (0xFFFFFFFF)));
...
void adlak_hal_smmu_cache_invalid(void *data, dma_addr_t iova)
{
	...
	struct io_region *region = padlak->hw_res.preg;
	union HAL_ADLAK_SMMU_INV_CTL_S d;
	u32 cnt;
	...
	adlak_write32(region, REG_ADLAK_0XD8, (u32)iova);
	adlak_write32(region, REG_ADLAK_0XD4, (u32)d.all);
...
static void adlak_parser_set_pend_timer(struct io_region *region, u32 time)
{
	union HAL_ADLAK_PS_PEND_EN_S d;
...
static void adlak_parser_set_apb_timeout(struct io_region *region, u32 time)
{
	// union HAL_ADLAK_PS_PEND_EN_S d;
	adlak_write32(region, REG_ADLAK_0X4, time);
}

static void adlak_hal_enable(void *data, u32 en)
{
	struct adlak_device *padlak = (struct adlak_device *)data;
	struct adlak_hw_info *phw_info =
		(struct adlak_hw_info *)padlak->hw_info;
	union HAL_ADLAK_ADLAK_EN_S d;
...
void adlak_hal_irq_enable(void *data, u32 en)
{
	struct adlak_device *padlak = (struct adlak_device *)data;
	struct adlak_hw_info *phw_info =
		(struct adlak_hw_info *)padlak->hw_info;
	...
	if (!en) {
		...
		adlak_write32(padlak->hw_res.preg, REG_ADLAK_0X18,
			      0xFFFF); // irq clear
		adlak_write32(padlak->hw_res.preg, REG_ADLAK_0X14,
			      phw_info->irq_cfg.mask);
	}
}
...
static void adlak_hal_get_revision(void *data)
{
	...
	AML_LOG_INFO("ADLAK HW Revision:%d.%d", phw_info->rev.bitc.major,
		     phw_info->rev.bitc.minor);
}

static void adlak_hal_get_all_regs(struct adlak_hw_stat *phw_stat)
{
	struct io_region *region = NULL;
	int idx;
	...
	for (idx = 0; idx < sizeof(phw_stat->hw_info->reg_lst) /
				    sizeof(phw_stat->hw_info->reg_lst[0]);
	     idx++) {
		phw_stat->regs_stat[idx] =
			adlak_read32(region, phw_stat->hw_info->reg_lst[idx]);
	}
}
...
static void adlak_all_regs_dump(struct adlak_hw_stat *phw_stat)
{
	...
	for (idx = 0; idx < sizeof(phw_stat->hw_info->reg_lst) /
				    sizeof(phw_stat->hw_info->reg_lst[0]);
	     idx++) {
		adlak_get_reg_name(phw_stat->hw_info->reg_lst[idx], reg_name,
				   sizeof(reg_name));
		adlak_os_printf("0x%-*x%-*s0x%08x", 6,
				phw_stat->hw_info->reg_lst[idx], 22, reg_name,
				phw_stat->regs_stat[idx]);
	}
}
...
u32 adlak_get_hw_status(struct adlak_hw_stat *phw_stat)
{
	...
	if (phw_stat->ps_status & ADLAK_IRQ_MASK_INVALID_IOVA) {
		phw_stat->smmu_err_dft_pa =
			adlak_read32(region, REG_ADLAK_0XDC);
		phw_stat->smmu_err_mdl_id =
			adlak_read32(region, REG_ADLAK_0XE0);
...
adlak_hal_get_irq_status(struct adlak_hw_stat *phw_stat)
{
	struct io_region *region = NULL;
	...
	if (phw_stat->irq_status.timeout)
	...
	phw_stat->irq_status.status_report =
		adlak_read32(region, REG_ADLAK_0X1C);
...
{
	AML_LOG_DEBUG("REG_ADLAK_0X10   : 0x%08X",
		      phw_stat->irq_status.irq_masked);
	AML_LOG_DEBUG("REG_ADLAK_0X18      : 0x%08X",
		      phw_stat->irq_status.irq_raw);
	AML_LOG_DEBUG("REG_ADLAK_0X1C   : 0x%08X",
		      phw_stat->irq_status.status_report);
	...
	AML_LOG_DEBUG("REG_ADLAK_0X60: 0x%08X",
	...
	if (phw_stat->ps_status & ADLAK_IRQ_MASK_INVALID_IOVA) {
		AML_LOG_DEBUG("REG_ADLAK_0XDC     : 0x%08X",
			      phw_stat->smmu_err_dft_pa);
		AML_LOG_DEBUG("REG_ADLAK_0XE0 : 0x%08X",
			      phw_stat->smmu_err_mdl_id);
		AML_LOG_DEBUG("REG_ADLAK_0XE4  : 0x%08X",
			      phw_stat->smmu_err_iova);
	}
...
static int adlak_hal_set_axisram(void *data)
{
	struct adlak_device *padlak = (struct adlak_device *)data;
	struct io_region *region = padlak->hw_res.preg;
	u32 pa_start, va_start, va_end, wrap_en;
	union HAL_ADLAK_AB_CTL_S d_ab;

	if (padlak->hw_res.adlak_sram_size != 0) {
		AML_LOG_INFO("AXI_SRAM_BASE = 0x%lx,size = 0x%lX",
			     (uintptr_t)padlak->hw_res.adlak_sram_pa,
			     (uintptr_t)padlak->hw_res.adlak_sram_size);
		wrap_en = padlak->hw_res.sram_wrap;
		pa_start = (u32)((uintptr_t)padlak->hw_res.adlak_sram_pa);
		va_start = (u32)((uintptr_t)padlak->hw_res.adlak_sram_pa);
		va_end = (u32)((uintptr_t)padlak->hw_res.adlak_sram_pa) +
			 (u32)((uintptr_t)padlak->hw_res.adlak_sram_size);

		if (wrap_en)
			va_end += va_end - va_start;

		AML_LOG_INFO("va_start = 0x%lx,va_end = 0x%lX",
...
static int adlak_hal_set_autoclock(void *data, u32 en)
{
	struct adlak_device *padlak = (struct adlak_device *)data;
	struct io_region *region = padlak->hw_res.preg;
...
static int adlak_hal_reset(void *data)
{
	int ret = ERR(EINVAL);
	struct adlak_device *padlak = (struct adlak_device *)data;
	//struct io_region *   region = NULL;
...
static int adlak_hal_start(void *data)
{
	struct adlak_device *padlak = (struct adlak_device *)data;
...
static int adlak_hal_stop(void *data)
{
	struct adlak_device *padlak = (struct adlak_device *)data;
...
int adlak_get_reg_name(int offset, char *buf, size_t buf_size)
{
	...
	case REG_ADLAK_0X8C:
		return adlak_os_snprintf(buf, buf_size, "%s",
					 "PS_MODULE_IDLE_STS");
	case REG_ADLAK_0X90:
...
int adlak_hw_init(void *data)
{
	...
	phw_info =
		adlak_os_zalloc(sizeof(struct adlak_hw_info), ADLAK_GFP_KERNEL);
	...
	phw_info->irq_cfg.mask_err =
		ADLAK_IRQ_MASK_PEND_TIMOUT | ADLAK_IRQ_MASK_APB_WAIT_TIMOUT |
		ADLAK_IRQ_MASK_PARSER_STOP_ERR | ADLAK_IRQ_MASK_INVALID_IOVA |
		ADLAK_IRQ_MASK_SW_TIMEOUT;

	phw_info->irq_cfg.mask_err =
		phw_info->irq_cfg.mask_err |
		(ADLAK_IRQ_MASK_PM_DRAM_OVF | ADLAK_IRQ_MASK_PM_FIFO_OVF |
		 ADLAK_IRQ_MASK_PM_ARBITER_OVF);
...
#endif
	phw_info->irq_cfg.mask =
		((phw_info->irq_cfg.mask_err ^ ADLAK_IRQ_MASK_SW_TIMEOUT) |
		 phw_info->irq_cfg.mask_normal);
...
int adlak_check_dev_is_idle(void *data)
{
	...
	cnt = 0;
	while (1) {
		idle_sts = adlak_read32(phw_info->region, REG_ADLAK_0X8C);
		if (idle_sts != 0xFFFFFFFF)
			AML_LOG_WARN("REG_ADLAK_0X8C   : 0x%08X", idle_sts);
		else
			break;
		adlak_os_udelay(1);
		cnt++;
		if (cnt > 30000) {
			AML_LOG_ERR("wait device idle timeout!");
			return ERR(EIO);
		}
	};
	return ERR(NONE);
...
{
	struct adlak_device *padlak = (struct adlak_device *)data;
	struct adlak_parser_storage *parser_storage =
		(struct adlak_parser_storage *)_parser_storage;
	struct io_region *region = padlak->hw_res.preg;
	...
	AML_LOG_DEBUG("get base addr     = [0x%08X]",
		      parser_storage->base_addr);
...
int adlak_hal_parser_resume(void *data, void *_parser_storage)
{
	...
	AML_LOG_DEBUG("restore base addr = [0x%08X]",
		      parser_storage->base_addr);
	...
	if (parser_storage->is_save_from_hw != 0) {
		adlak_write32(region, REG_ADLAK_0X78,
			      parser_storage->finish_id);
		adlak_write32(region, REG_ADLAK_0X60,
			      parser_storage->timestamp);
		adlak_write32(region, REG_ADLAK_0X90,
			      parser_storage->dbg_sw_id);
		AML_LOG_DEBUG("restore finish_id = [0x%08X]",
			      parser_storage->finish_id);
	}
