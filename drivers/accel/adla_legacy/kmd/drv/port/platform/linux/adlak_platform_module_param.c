...
module_param(register_mem_base, uint, 0644);
MODULE_PARM_DESC(register_mem_base,
		 "[ignored if device tree enabled]AHB register base physic address");

module_param(register_mem_size, uint, 0644);
MODULE_PARM_DESC(register_mem_size,
		 "[ignored if device tree enabled]AHB register size in bytes");

module_param(contiguous_mem_base, uint, 0644);
MODULE_PARM_DESC(contiguous_mem_base,
		 "[ignored if device tree enabled]default base physical address of adlak memory heap");

module_param(contiguous_mem_size, uint, 0644);
MODULE_PARM_DESC(contiguous_mem_size,
		 "[ignored if device tree enabled]default size of adlak memory heap in bytes");

module_param(contiguous_sram_base, uint, 0644);
MODULE_PARM_DESC(contiguous_sram_base,
		 "[ignored if device tree enabled]default base physical address of adlak sram heap");

module_param(contiguous_sram_size, uint, 0644);
MODULE_PARM_DESC(contiguous_sram_size,
...
MODULE_PARM_DESC(dpm_period,
		 "the check-period of the dynamic power management in ms");
...
MODULE_PARM_DESC(share_swap,
		 "share swap buffer between different models, disabled by default");
