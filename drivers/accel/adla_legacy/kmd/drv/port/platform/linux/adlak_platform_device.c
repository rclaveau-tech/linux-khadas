...
static bool misc_dev_en;
static int adlak_major = -1;
struct class *adlak_class;
...
static int adlak_create_cdev(struct adlak_device *padlak)
{
	...
	cdev_init(&padlak->cdev, &adlak_fops);
	...
	sysdev = device_create(padlak->class, padlak->dev, padlak->devt, padlak,
			       DEVICE_NAME "%d", id);
	if (IS_ERR(sysdev)) {
...
#ifdef CONFIG_PM
static int adlak_platform_sys_suspend(struct platform_device *pdev,
				      pm_message_t state)
{
	struct adlak_device *padlak = platform_get_drvdata(pdev);
	int pm_suspend;
...
#ifndef CONFIG_OF
	if (adlak_platform_device_uninit() < 0)
		AML_LOG_ERR("Soc platform driver uninit failed.");
#endif
	adlak_class_release();
}
module_init(adlak_module_init) module_exit(adlak_module_exit)
	MODULE_LICENSE("GPL");
