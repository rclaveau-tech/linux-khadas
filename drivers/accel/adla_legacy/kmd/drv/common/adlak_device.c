...
int adlak_device_deinit(struct adlak_device *padlak)
{
	...
	adlak_os_mutex_unlock(&padlak->dev_mutex);
	/*adlak_dev_inference_deinit(): the inference thread will internally call the dev_mutex*/
	adlak_dev_inference_deinit(padlak);
...
int adlak_irq_proc(struct adlak_device *const padlak)
{
	...
	cmq_buf_info = (struct adlak_cmq_buffer *)
			       ptask->context->pmodel_attr->cmq_buffer;
	...
	if (unlikely(irqstatus->irq_masked == 0)) {
		AML_LOG_WARN("irq_masked[0x%08X] irq_raw[0x%08X]",
			     irqstatus->irq_masked, irqstatus->irq_raw);
		return -1;
	}
