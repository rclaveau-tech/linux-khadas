					...
					void *pmm_info)
{
	...
	list_for_each_prev_safe(node, node_tmp, &context->sbuf_list) {
		/* Due to the special application scenario,
		 * the target can be found faster by looking from
		...
		if (context_buf) {
			...
			if (pmm_info_tmp->uid ==
			    ((struct adlak_mem_handle *)pmm_info)->uid) {
...
int adlak_context_attach_buf(struct adlak_context *context, void *mm_info)
{
	...
	new_sbuf =
		adlak_os_zalloc(sizeof(struct context_buf), ADLAK_GFP_KERNEL);
	if (!new_sbuf) {
