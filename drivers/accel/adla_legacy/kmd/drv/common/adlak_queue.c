...
#if ADLAK_DEBUG
static int print_task_info(char *buf, ssize_t buf_size,
			   struct adlak_task *ptask)
{
	...
	case ADLAK_SUBMIT_STATE_IDLE:
		adlak_os_snprintf(state_str, sizeof(state_str), "idle");
		break;
	...
	return adlak_os_snprintf(&buf[0], buf_size,
				 "%-*d%-*d%-*d%-*d%-*s%-*d\n", 12,
				 ptask->context->net_id, 12, ptask->invoke_idx,
				 20, ptask->invoke_start_idx, 20,
				 ptask->invoke_end_idx, 12, state_str, 12,
				 ptask->flag);
}
...
"----------------------------------------------------------------------------------------------";
	...
	if (!number) {
		ret += adlak_os_snprintf(buf + ret, buf_size - ret,
					 "No task.\n");
	}
	ret += adlak_os_snprintf(buf + ret, buf_size - ret, "%s\n", boundary);
...
int adlak_debug_invoke_list_dump(struct adlak_device *padlak, u32 debug)
{
	...
	ret += adlak_os_snprintf(buf + ret, buf_size - ret,
				 "%-*s%-*s%-*s%-*s%-*s%-*s\n", 12, "net_id", 12,
				 "invoke_id", 20, "invoke_start_id", 20,
				 "invoke_end_id", 12, "state", 12, "flag");
