	...
	if (pdpm_info) {
		...
		case ADLAK_DPM_STRATEGY_DOWN:
			...
			if (pdpm_info->freq_cfg_idx >=
			    (ADLAK_ARRAY_SIZE(pdpm_info->freq_cfg_list[0]) -
			     1)) {
				pdpm_info->freq_cfg_idx =
		...
		pdpm_info->core_freq_expect =
			pdpm_info->freq_cfg_list[0][pdpm_info->freq_cfg_idx];
		pdpm_info->axi_freq_expect =
			pdpm_info->freq_cfg_list[1][pdpm_info->freq_cfg_idx];
...
void adlak_dpm_clk_update(void *data, int core_freq, int axi_freq)
{
	...
	AML_LOG_DEBUG("%s axi_freq=%d, core_freq=%d", __func__, axi_freq,
		      core_freq);
	...
	AML_LOG_DEBUG("%s pdpm_info->freq_cfg_idx=%d", __func__,
		      pdpm_info->freq_cfg_idx);
#endif
}
...
int adlak_dpm_init(void *data)
{
	...
	pdpm_info = adlak_os_zalloc(sizeof(struct adlak_power_info),
				    ADLAK_GFP_KERNEL);
	if (!pdpm_info) {
		ret = ERR(ENOMEM);
		goto err;
	...
	for (i = 0; i < ADLAK_ARRAY_SIZE(pdpm_info->freq_cfg_list[0]); i++) {
		switch (i) {
		case 0:
			pdpm_info->freq_cfg_list[0][i] =
				padlak->clk_core_freq_set;
			pdpm_info->freq_cfg_list[1][i] =
				padlak->clk_axi_freq_set;
			break;
		case 1:
			pdpm_info->freq_cfg_list[0][i] =
				padlak->clk_core_freq_set / 2;
			pdpm_info->freq_cfg_list[1][i] =
				padlak->clk_axi_freq_set / 2;
			break;
		case 2:
			pdpm_info->freq_cfg_list[0][i] =
				padlak->clk_core_freq_set / 3;
			pdpm_info->freq_cfg_list[1][i] =
				padlak->clk_axi_freq_set / 3;
			break;
		case 3:
			pdpm_info->freq_cfg_list[0][i] =
				padlak->clk_core_freq_set / 4;
			pdpm_info->freq_cfg_list[1][i] =
				padlak->clk_axi_freq_set / 4;
			break;
		default:
	...
	pdpm_info->axi_freq_expect =
		pdpm_info->freq_cfg_list[1][pdpm_info->freq_cfg_idx];
	pdpm_info->core_freq_cur = pdpm_info->freq_cfg_list[0]
					      [(ADLAK_ARRAY_SIZE(pdpm_info->freq_cfg_list[0]) - 1)];
	pdpm_info->axi_freq_cur = pdpm_info->freq_cfg_list[1]
					     [(ADLAK_ARRAY_SIZE(pdpm_info->freq_cfg_list[0]) - 1)];

	AML_LOG_DEBUG("%s axi_freq=%d, core_freq=%d", __func__,
		      pdpm_info->axi_freq_cur, pdpm_info->core_freq_cur);
	return 0;
err:
...
int adlak_dmp_get_efficiency(void *data)
{
	...
	efficiency = (pwq->dev_inference.cnt_busy * 100) /
		     (pwq->dev_inference.cnt_elapsed);
	AML_LOG_INFO("adlak efficiency %d/%d = %d%%",
		     pwq->dev_inference.cnt_busy,
