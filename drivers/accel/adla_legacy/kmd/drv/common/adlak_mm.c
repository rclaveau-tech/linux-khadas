...
int adlak_mem_alloc_request(struct adlak_context *context,
			    struct adlak_buf_req *pbuf_req)
{
	...
	AML_LOG_DEBUG("mem_alloc_request size:0x%lX bytes",
		      (uintptr_t)pbuf_req->bytes);
	...
	AML_LOG_DEBUG("cpu_addr_user=0x%lX, ",
	...
	pbuf_req->mem_handle = (u64)(uintptr_t)mm_info;
...
int adlak_mem_free_request(struct adlak_context *context, u64 mem_handle)
{
	int ret = 0;
	struct adlak_mem_handle *mm_info =
		(struct adlak_mem_handle *)(uintptr_t)mem_handle;
	struct adlak_mem_handle *pmm_info_hd;
	struct adlak_mem_obj *mem = &memory;

	if (context->invoke_cnt) {
		ret = -1;
		goto err;
	}

	// step1: dettach from the mem list
	pmm_info_hd = (struct adlak_mem_handle *)adlak_context_dettach_buf(context,
									   (void *)mm_info);
	if (!pmm_info_hd) {
		ret = -1;
				 ...
				 struct adlak_extern_buf_info *pbuf_req)
{
	...
	if (!pbuf_req->buf_type) { // physical addr type
		AML_LOG_DEBUG("this extern memory type is physical.");

		mm_info = mem->ops.attach(&context->smmu_attr, pbuf_req);
	} else if (pbuf_req->buf_type == 1) { //  dma handle addr type
		//  AML_LOG_DEBUG("this extern memory type is dma handle.");
		ret = -1;
		AML_LOG_ERR("This memory type[%u] is unsupported Temporarily.",
			    pbuf_req->buf_type);
		goto err;

	} else {
		ret = -1;
		AML_LOG_ERR("This memory type[%u] is not support.",
			    pbuf_req->buf_type);
		goto err;
	}
...
int adlak_ext_mem_dettach_request(struct adlak_context *context, u64 mem_handle)
{
	int ret = 0;
	struct adlak_mem_handle *mm_info =
		(struct adlak_mem_handle *)(uintptr_t)mem_handle;
...
int adlak_mem_free_all_context(struct adlak_context *context)
{
	...
	if (!list_empty(&context->sbuf_list)) {
		list_for_each_entry_safe(pcontext, pcontext_tmp,
					 &context->sbuf_list, head) {
			if (pcontext) {
				list_del(&pcontext->head);
				pmm_info_hd = pcontext->mm_info;
				if (pmm_info_hd->mem_src !=
				    ADLAK_ENUM_MEMSRC_EXT_PHYS) {
					context->mem_alloced -=
						pmm_info_hd->req.bytes;
					mem->ops.free(&context->smmu_attr,
						      pmm_info_hd);
				} else {
					mem->ops.dettach(&context->smmu_attr,
							 pmm_info_hd);
				}
...
int adlak_mem_flush_request(struct adlak_context *context,
			    struct adlak_buf_flush *pflush_desc)
{
	int ret = 0;
	struct adlak_mem_handle *mm_info = NULL;
	...
	mm_info =
		(struct adlak_mem_handle *)(uintptr_t)(pflush_desc->mem_handle);
	...
	if (!target_buf) {
		...
		case FLUSH_FROM_DEVICE:
			// invalid cache
			adlak_invalid_cache(padlak, mm_info,
					    &sync_cache_extern);
			break;
		default:
			break;
...
struct adlak_mem_handle *adlak_cmq_buf_alloc(struct adlak_context *context,
					     size_t size)
{
	...
	if (size > 0x10000000) {
		AML_LOG_ERR("allocate cmq size %08lX grater than 256Mbytes",
			    (uintptr_t)size);
		return cmq_mm_info;
	}
	buf_req.bytes = size;
	buf_req.mem_type = ADLAK_ENUM_MEMTYPE_INNER;
	/* To avoid constantly sync the cache of cmq, it is recommended that cmq use uncache buffer
	 */
	...
	if (!cmq_mm_info) {
		...
		AML_LOG_INFO("cmq buffer iova_addr=0x%lX",
			     (uintptr_t)cmq_mm_info->iova_addr);
	}
	return cmq_mm_info;
}

void adlak_cmq_buf_free(struct adlak_context *context,
...
int adlak_public_cmq_buf_init(struct adlak_device *padlak)
{
	...
	AML_LOG_INFO("alloc buffer for cmq(public),size=0x%08x",
		     cmq_buf_info->size);
	cmq_buf_info->cmq_mm_info =
		adlak_cmq_buf_alloc(NULL, cmq_buf_info->size);
...
int adlak_mem_mmap(struct adlak_context *context, void *const vma, u64 uid)
{
	int ret = 0;
...
void adlak_mem_create_smmu(void *pptr_smmu, u8 type)
{
	struct adlak_mem_obj *mem = &memory;
...
void adlak_mem_destroy_smmu(void *pptr_smmu, u8 type)
{
	struct adlak_mem_obj *mem = &memory;
...
int adlak_flush_cache(struct adlak_device *padlak,
		      struct adlak_mem_handle *mm_info,
...
int adlak_invalid_cache(struct adlak_device *padlak,
			struct adlak_mem_handle *mm_info,
			struct adlak_sync_cache_ext_info *sync_cache_ext_info)
{
	struct adlak_mem_obj *mem = &memory;
...
{
	struct adlak_caps_desc *uapi_caps =
		(struct adlak_caps_desc *)padlak->dev_caps.data;
	...
	if (mem->use_smmu) {
		...
		uapi_caps->iova_max_size =
			mem_usage.pool_size; /* tha maximum vaddr of smmu*/
		uapi_caps->iova_free_size =
			mem_usage.pool_size -
			(mem_usage.alloced_umd +
			 mem_usage.alloced_kmd); /* tha free size of vaddr*/
	}
}

u64 adlak_mem_get_smmu_entry(struct adlak_context *context, u8 type)
{
	struct adlak_mem_obj *mem = &memory;
	...
	if (mem->use_smmu) {
		...
		return mem->ops.get_smmu_entry(&context->smmu_attr,
					       type);
	}
