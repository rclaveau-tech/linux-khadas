...
#define ADLAK_TASK_CANCELED BIT(0)
...
struct adlak_submit_dep_fixup {
	...
	u32 id_shift;
	u32 id_mask;
	u32 dep_modes[ADLAK_DEPENDENCY_MODE_COUNT];
	s32 mode_loc;
	u32 mode_shift;
	u32 mode_mask;
};
...
struct adlak_submit_task {
	u32 active_modules;
	u32 output_modules;
	u32 memory_access_modules;
	s32 config_offset;
	s32 config_size;
	s32 addr_fixup_offset;
	s32 addr_fixup_count;
	s32 dep_info_offset;
	s32 dep_info_count;
	s32 reg_fixup_offset;
	s32 reg_fixup_count;
	s32 start_pwe_flid;
	s32 start_pwx_flid;
	s32 start_rs_flid;
	s32 memory_access_types[12]; /*not used in kmd*/
	u32 fence_modules;
	s32 dependency_mode;
...
struct adlak_model_attr {
	...
	s32 hw_layer_first; // indicate the first layer of hardware
	s32 hw_layer_last; // indicate the last layer of hardware
	u32 hw_layer_last_in_first_smmu_table; // the last hardware layer in the first smmu table
	struct adlak_pm_cfg pm_cfg;
	struct adlak_pm_state pm_stat;
	s32 invoke_count;

	// In order to avoid continuous application and release of task memory
	struct adlak_task *invoke_attr_rsv;

	struct adla_context_cmq_offset
		*cmq_offsets; // Point to rsv_pool_base+offset
	s32 cmq_buffer_type;
	u32 cmq_size_expected; // the buffer size required to store the command
	u32 size_max_in_layer;
	struct adlak_cmq_buffer *cmq_buffer;
	s32 hw_parser_v2_support; // If non-zero, indicates that the hardware supports parser_v2
