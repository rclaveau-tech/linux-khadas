...
/***************************** Include Files *********************************/
#include "adlak_dpm.h"
#include "../port/platform/adlak_reg.h"
...
static struct adlak_workqueue *g_adlak_pwq;

/************************** Function Prototypes ******************************/

static void adlak_irq_bottom_half(struct adlak_device *padlak,
				  int *device_state, struct adlak_task *ptask);

static int adlak_submit_wait(struct adlak_dev_inference *pinference,
			     struct adlak_task *ptask)
{
	...
	AML_LOG_INFO("set submit hw_timeout_ms=%u ms",
		     ptask->context->pmodel_attr->hw_timeout_ms);
	if (ptask->context->pmodel_attr->hw_timeout_ms) {
		do {
			ret = adlak_os_sema_take_timeout
				(pinference->sem_irq, ptask->context->pmodel_attr->hw_timeout_ms);
			if (ERR(NONE) == ret) {
				AML_LOG_INFO("%s\n", "sema_take success");
				timeout = false;
				break;
...
static void adlak_debug_print_device_state(u32 device_state)
{
#if ADLAK_DEBUG
	switch (device_state) {
	...
	if (net_id < 0) {
		...
		if (pwq->pending_num > 0) {
			list_for_each_entry_safe(ptask, ptask_tmp,
						 &pwq->pending_list, head) {
				if (net_id == ptask->context->net_id) {
					if (CONTEXT_STATE_CLOSED ==
					    ptask->context->state) {
			...
			if (find != 1) {
				...
				if (adlak_check_netid_is_valid(pwq->padlak, *net_id_pre)) {
...
static void *adlak_dev_inference_cb(void *args)
{
	...
	struct adlak_os_thread_t *pthrd = &pwq->dev_inference.thrd_inference;
	struct adlak_task *ptask_sch_cur = NULL, *ptask_sch_pre = NULL;
	s32 net_id_invoke_previous = -1;
	...
	int pm_suspend;
#endif
	ret = adlak_os_timer_init(&pwq->dev_inference.dpm_timer,
				  adlak_dpm_timer_cb, NULL);
	if (ret) {
	...
	AML_LOG_WARN("dpm_timer_period: %d\n",
		     pwq->dev_inference.dpm_period_set);
	adlak_os_timer_add(pwq->dev_inference.dpm_timer,
			   pwq->dev_inference.dpm_period_set);
#endif
	...
	while (!pthrd->thrd_should_stop) {
		...
		case ADLAK_INFERENCE_STATE_RESUME:
			// do resume
			if (dpm_stategy != ADLAK_DPM_STRATEGY_MAX) {
		...
		case ADLAK_INFERENCE_STATE_WQ_CHECK:
			...
			pending_num = pwq->pending_num;
			adlak_os_mutex_unlock(&pwq->wq_mutex);
			if (!valid_num && pending_num > 0) {
				ASSERT(padlak->share_swap_en);
				adlak_os_sema_give(pwq->wk_update);
				adlak_os_thread_yield();
				/* must sleep to free up CPU,if the thread's priority is
				 * particularly high.
				 */
				adlak_os_msleep(1);
			}
			...
			// check workqueue
			if (valid_num > 0) {
				if (dpm_stategy == ADLAK_DPM_STRATEGY_MIN) {
					pwq->dev_inference.wq_idle_cnt = 0;
					inference_state =
						ADLAK_INFERENCE_STATE_RESUME;
					adlak_os_sema_give(pwq->wk_update);
		...
		case ADLAK_INFERENCE_STATE_SLEEP_CHECK:
			// do_sleep
			if (!pending_num) { // valid_num must be 0
				AML_LOG_INFO("nothing need to do!\n");
				if (pwq->dev_inference.wq_idle_cnt >= 10) {
			...
			if (pm_suspend && device_state == ADLAK_DEVICE_IDLE) {
				dpm_stategy = ADLAK_DPM_STRATEGY_MIN;
				adlak_dpm_stage_adjust(padlak,
						       ADLAK_DPM_STRATEGY_MIN);
				adlak_os_mutex_lock(&padlak->dev_mutex);
		...
		case ADLAK_INFERENCE_STATE_POST_TASK:
			...
			ASSERT(!ptask_sch_cur);
			ret = adlak_queue_schedule_update(padlak, &ptask_sch_cur,
							  net_id_invoke_previous);
			if (ret) {
				inference_state =
					ADLAK_INFERENCE_STATE_WQ_CHECK;
				break;
			...
			pwq->ptask_sch_cur = ptask_sch_cur;
			pwq->submit_num++;
			if (!ptask_sch_cur->context->pmodel_attr
					 ->hw_parser_v2_support) {
			...
			if (padlak->share_swap_en) {
				if (ptask_sch_cur->invoke_end_idx <
				    ptask_sch_cur->context->pmodel_attr
					    ->hw_layer_last) {
					net_id_invoke_previous =
						ptask_sch_cur->context->net_id;
				}
			}
		...
		case ADLAK_INFERENCE_STATE_REPORT_TASK_PRE2:
			if (ptask_sch_pre) {
				adlak_os_mutex_lock(&pwq->wq_mutex);
				ret = adlak_queue_update_task_state
					(padlak, (struct adlak_task *)ptask_sch_pre);
				adlak_os_mutex_unlock(&pwq->wq_mutex);
				if (ret == 1) {
					/*give signal to umd*/
					adlak_to_umd_sinal_give(ptask_sch_pre->context->wait);
				}
				// report state
				ptask_sch_pre = NULL;
			}
			if (ADLAK_INFERENCE_STATE_REPORT_TASK_PRE ==
			    inference_state) {
				inference_state =
					ADLAK_INFERENCE_STATE_SUBMIT_WAIT;
			} else {
		...
		case ADLAK_INFERENCE_STATE_SUBMIT_WAIT:
			// wait until finished
			if (ERR(NONE) != adlak_submit_wait(&pwq->dev_inference,
							   ptask_sch_cur)) {
				AML_LOG_ERR("%s\n", "submit timeout");
				ptask_sch_cur->hw_stat.irq_status.timeout =
					true;
			} else {
				AML_LOG_INFO("%s\n", "submit success");
			}
			adlak_irq_bottom_half(padlak, &device_state,
					      ptask_sch_cur);
		...
		case ADLAK_INFERENCE_STATE_REPORT_TASK_CUR:
			...
			if (ret == 1) {
				/*give signal to umd*/
				adlak_to_umd_sinal_give(ptask_sch_cur->context->wait);
			}
			if (ret >= 0) {
				ptask_sch_cur = NULL;
				pwq->ptask_sch_cur = ptask_sch_cur;
...
static void adlak_emu_irq_cb(adlak_os_timer_cb_t t)
{
	...
	ptask->context->pmodel_attr->cmq_buffer->cmq_rd_offset =
		phw_stat->ps_rbf_rpt;
...
#if defined(CONFIG_ADLAK_EMU_EN) && (CONFIG_ADLAK_EMU_EN == 1)
	ret = adlak_os_timer_init(&pinference->emu_timer, adlak_emu_irq_cb,
				  NULL);
	if (ret)
...
{
	union HAL_ADLAK_REPORT_STATUS_S d;
	...
	if (d.bitc.hang_vlc_decoder) {
		AML_LOG_WARN(" [7]: vlc decoder hang rpid = %d.",
			     d.bitc.vlc_decoder_rpid);
	}
				  ...
				  int *device_state, struct adlak_task *ptask)
{
	...
	phw_stat = &ptask->hw_stat;
	adlak_profile_stop(padlak, ptask->context,
			   &ptask->context->pmodel_attr->pm_cfg,
			   &ptask->context->pmodel_attr->pm_stat,
			   &ptask->profilling,
			   (ptask->invoke_end_idx >=
			    ptask->context->pmodel_attr->hw_layer_last) ?
				   1 :
				   0);
	...
	if (phw_info->irq_cfg.mask_normal & phw_stat->irq_status.irq_masked) {
		*device_state = ADLAK_DEVICE_IDLE;
		if (ptask->time_stamp == phw_stat->irq_status.time_stamp) {
			...
			AML_LOG_INFO("submit_finished,IRQ status[0x%08X].",
				     phw_stat->irq_status.irq_masked);
		} else {
			*device_state = ADLAK_DEVICE_ERR;
			if (ptask->context->state != CONTEXT_STATE_CLOSED) {
				AML_LOG_ERR("time_stamp_expect[%d],time_stamp_get[%d]",
					    ptask->time_stamp,
	...
	} else if (phw_info->irq_cfg.mask_err &
		   phw_stat->irq_status.irq_masked) {
		*device_state = ADLAK_DEVICE_ERR;
		AML_LOG_ERR("IRQ status[0x%08X].",
			    phw_stat->irq_status.irq_masked);
		...
		*device_state = ADLAK_DEVICE_ERR;
		if (ptask->context->state != CONTEXT_STATE_CLOSED) {
			AML_LOG_ERR("Not support status[0x%08X]!",
				    phw_stat->irq_status.irq_masked);
			ASSERT(0);
		} else {
