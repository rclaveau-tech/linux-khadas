...
static struct adlak_simple_bitmap *ptr_mem_uid;
			   ...
			   struct adlak_mem_exclude *exclude_area)
{
	...
	size_t exclude_bitmap_count, exclude_area_start_aligned,
		exclude_area_size_aligned;

	bitmap_size = BITS_TO_LONGS(pool_size >> ADLAK_MM_POOL_PAGE_SHIFT) *
		      sizeof(unsigned long);
	AML_LOG_DEBUG("%s pool_size 0x%lX bitmap_size 0x%lX", __func__,
		      (uintptr_t)pool_size, (uintptr_t)bitmap_size);
	...
	if (exclude_area->size) {
		exclude_area_start_aligned =
			(exclude_area->start & ADLAK_MM_POOL_PAGE_MASK);
		exclude_area_size_aligned =
			ADLAK_ALIGN(exclude_area->start + exclude_area->size,
				    ADLAK_MM_POOL_PAGE_SIZE) -
			exclude_area_start_aligned;

		exclude_area_start_aligned =
			exclude_area_start_aligned - pool->addr_base;
		exclude_bitmap_count = exclude_area_size_aligned >>
				       ADLAK_MM_POOL_PAGE_SHIFT;
		start = (exclude_area_start_aligned >>
			 ADLAK_MM_POOL_PAGE_SHIFT);

		if (start < pool->bits) {
			AML_LOG_INFO
	("set exclude area in bitmap pool: start 0x%08lX, count 0x%08lX ",
	(uintptr_t)(start),
	(uintptr_t)exclude_bitmap_count);
			bitmap_set(pool->bitmap, start, exclude_bitmap_count);
		}
	}

	AML_LOG_INFO
("mm_pool size = 0x%lX byte,bitmap_size = 0x%lX byte,bitmap_maxno = 0x%lX,pool_addr_base = 0x%lX.",
(uintptr_t)pool_size, (uintptr_t)bitmap_size,
(uintptr_t)pool->bits, (uintptr_t)pool->addr_base);
...
{
#if ADLAK_DEBUG
	u32 bitmap_size = pool->bitmap_size;
	u8 *addr = pool->bitmap;
	size_t i;

	for (i = bitmap_size - 1; i >= 15;) {
		AML_LOG_WARN
("offset%08lX~%08lX:%02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X%02X",
			(uintptr_t)i, (uintptr_t)(i - 15), addr[i - 0],
			addr[i - 1], addr[i - 2], addr[i - 3], addr[i - 4],
			addr[i - 5], addr[i - 6], addr[i - 7], addr[i - 8],
			addr[i - 9], addr[i - 10], addr[i - 11], addr[i - 12],
			addr[i - 13], addr[i - 14], addr[i - 15]);
...
unsigned long adlak_alloc_from_bitmap_pool(struct adlak_mm_pool_priv *pool,
					   size_t size)
{
	size_t start = 0;
	size_t bitmap_count;
...
(uintptr_t)bitmap_count);
	...
	start = bitmap_find_next_zero_area(pool->bitmap, pool->bits, 0,
					   bitmap_count, 0);
	if (start >= pool->bits) {
		AML_LOG_WARN
("Didn't find zero area from mem-pool!\nbitmap_maxno = %lu,bitmap_usedno = %lu,bitmap_count = %lu",
...
void adlak_free_to_bitmap_pool(struct adlak_mm_pool_priv *pool,
			       dma_addr_t addr_offset, size_t size)
{
	int bitmap_count;
	...
	bitmap_count = size >> ADLAK_MM_POOL_PAGE_SHIFT;
	adlak_os_mutex_lock(&pool->lock);
	bitmap_clear(pool->bitmap,
		     (addr_offset - pool->addr_base) / ADLAK_MM_POOL_PAGE_SIZE,
		     bitmap_count);
	pool->used_count -= bitmap_count;
	adlak_os_mutex_unlock(&pool->lock);
}

void adlak_set_bitmap_pool(struct adlak_mm_pool_priv *pool,
			   dma_addr_t addr_offset, size_t size)
{
	int bitmap_count;
...
/**
 * adlak_bitmap_find_next_zero_area_reverse - find a contiguous aligned zero area
 * @map: The address to base the search on
 * @pool_size: byte of bitmap
...
static signed long
adlak_bitmap_find_next_zero_area_reverse(void *map, signed long pool_size,
					 ...
					 signed long nr)
{
	...
	for (i = length - 1; i >= 0; --i) {
		...
		if (bitmap[i] == ulong_max) {
			...
			consecutive_zeros_start =
				-1; /// Reset start index of consecutive zeros
			continue;
		}
		...
		for (j = BITS_PER_LONG - 1; j >= 0; --j) {
			if (!(bitmap[i] & (1ul << j))) {
				...
				consecutive_zeros =
					0; // Reset consecutive zero count
				consecutive_zeros_start =
					-1; // Reset start index of consecutive zeros
			}
...
unsigned long adlak_alloc_from_bitmap_pool_reverse(struct adlak_mm_pool_priv *pool,
						   size_t size)
{
	size_t start = 0;
	size_t bitmap_count;
...
(uintptr_t)bitmap_count);
	...
	start = adlak_bitmap_find_next_zero_area_reverse(pool->bitmap, pool->bitmap_size,
							 pool->bits, 0, bitmap_count);
	if (start >= pool->bits) {
		AML_LOG_WARN
("Didn't find zero area from mem-pool!\nbitmap_maxno = %lu,bitmap_usedno = %lu,bitmap_count = %lu",
(uintptr_t)pool->bits, (uintptr_t)pool->used_count,
...
int adlak_mem_uid_init(void)
{
	...
	if (!ptr_mem_uid) {
		ptr_mem_uid =
			adlak_os_zalloc(sizeof(*ptr_mem_uid), ADLAK_GFP_KERNEL);
...
u64 adlak_mem_uid_alloc(void)
{
	u64 uid = ADLAK_INVALID_ADDR;
	int id;
...
void adlak_mem_uid_free(u64 uid)
{
	int id;
...
int adlak_simple_bitmap_pool_init(struct adlak_simple_bitmap *p_bitmap)
{
	...
	u32 pool_size =
		BITS_TO_LONGS(p_bitmap->size) * sizeof(unsigned long);
