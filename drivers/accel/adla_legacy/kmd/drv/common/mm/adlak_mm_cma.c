...
static struct adlak_mem_cma *ptr_mm_cma;
/************************** Function Prototypes ******************************/
...
static int adlak_mem_pools_mgr_init(struct adlak_mem_exclude *exclude_area)
{
	...
	return adlak_bitmap_pool_init(&ptr_mm_cma->mem_pool->bitmap_area,
				      pool_size, exclude_area);
}
...
adlak_alloc_share_buf_from_bitmap_pool_reverse(struct adlak_mem *mm,
					       size_t size)
{
	...
	AML_LOG_DEBUG("[share mem]need  bitmap_count %lu",
		      (uintptr_t)bitmap_count);
	// get max value
	bitmap_count = ((bitmap_count) < (share_buf->bitmap_count_max) ?
				(share_buf->bitmap_count_max) :
				(bitmap_count));
	if ((bitmap_count - share_buf->bitmap_count_max) >
	    (pool->bits - pool->used_count)) {
		AML_LOG_WARN
("Didn't find zero area from mem-pool!\nbitmap_maxno = %lu,bitmap_usedno = %lu,bitmap_count = %lu",
...
(uintptr_t)(bitmap_count - share_buf->bitmap_count_max));
	...
	if (share_buf->bitmap_count_max) {
		...
		AML_LOG_DEBUG("[share mem]clear bitmap from %lu,bitmap_count %lu\n",
			      (uintptr_t)share_buf->offset_start,
			      (uintptr_t)share_buf->bitmap_count_max);
		bitmap_clear(pool->bitmap, share_buf->offset_start,
			     share_buf->bitmap_count_max);
	}
	AML_LOG_DEBUG("[share mem]real alloc  bitmap_count %lu",
		      (uintptr_t)bitmap_count);

	search_start = pool->bits - bitmap_count;
	start = bitmap_find_next_zero_area(pool->bitmap, pool->bits,
					   search_start, bitmap_count, 0);
...
(uintptr_t)bitmap_count);
	...
	share_buf->bitmap_count_max =
		bitmap_count; // update the share_buf->bitmap_count
	pool->used_count += share_buf->bitmap_count_max;

	share_buf->offset_start = start;

	AML_LOG_DEBUG("start = %lu, bitmap_maxno = %lu,bitmap_usedno = %lu,bitmap_count = %lu",
		      (uintptr_t)start, (uintptr_t)pool->bits,
		      (uintptr_t)pool->used_count, (uintptr_t)bitmap_count);
	bitmap_set(pool->bitmap, start, bitmap_count);
	addr_offset = (unsigned long)(pool->addr_base +
				      start * ADLAK_MM_POOL_PAGE_SIZE);
...
ret:
	if (addr_offset == ADLAK_INVALID_ADDR)
...
static int adlak_malloc_from_mem_pool(struct adlak_mem *mm,
				      struct adlak_mem_handle *mm_info)
{
	...
	if (!(mm_info->req.mem_type & ADLAK_ENUM_MEMTYPE_INNER_SHARE)) {
		...
		start =
	adlak_alloc_share_buf_from_bitmap_pool_reverse(mm, (size_t)mm_info->req.bytes);
	}
	...
	mm_info->cpu_addr =
		(void *)((dma_addr_t)ptr_mm_cma->mem_pool->cpu_addr_base +
			 (dma_addr_t)start);
	mm_info->dma_addr =
		ptr_mm_cma->mem_pool->dma_addr_base + (dma_addr_t)start;
	mm_info->phys_addr =
		ptr_mm_cma->mem_pool->phys_addr_base + (phys_addr_t)start;
...
static void adlak_free_to_mem_pool(struct adlak_mem *mm,
				   struct adlak_mem_handle *mm_info)
{
	unsigned long start;

	if (mm_info->phys_addr) {
		start = mm_info->phys_addr -
			ptr_mm_cma->mem_pool->phys_addr_base;
...
static void adlak_free_cma_region_nocache(void)
{
	...
	if (ptr_mm_cma->mem_pool->cpu_addr_base) {
		dma_free_coherent(ptr_mm_cma->dev, ptr_mm_cma->mem_pool->size,
				  ptr_mm_cma->mem_pool->cpu_addr_base,
				  ptr_mm_cma->mem_pool->dma_addr_base);
	}
...
static int adlak_alloc_cma_region_nocache(void)
{
	...
	size_dec = size / 16;
	while (try--) {
		vaddr = dma_alloc_coherent(ptr_mm_cma->dev, (size_t)size,
					   &dma_hd, ADLAK_GFP_KERNEL);
		if (!vaddr) {
			AML_LOG_ERR("DMA alloc coherent failed: pa 0x%lX, size = %lu\n",
				    (uintptr_t)dma_hd, (uintptr_t)size);
			size = size - size_dec;
		} else {
			break;
		}
	...
	ptr_mm_cma->mem_pool = adlak_os_zalloc(sizeof(struct adlak_mem_pool_info),
					       ADLAK_GFP_KERNEL);
	if (ADLAK_IS_ERR_OR_NULL(ptr_mm_cma->mem_pool))
		goto err_alloc;
	...
	ptr_mm_cma->mem_pool->phys_addr_base =
		dma_to_phys(ptr_mm_cma->dev, dma_hd);
...
static void adlak_free_share_through_dma(struct adlak_mem *mm,
					 struct adlak_mem_handle *mm_info)
{
	if (mm_info->cpu_addr) {
		ptr_mm_cma->share_buf.ref_cnt--;
		adlak_os_printf("%s: dma_addr= 0x%lX,  phys_addr= 0x%lX,ref_cnt=%lu\n",
				__func__, (uintptr_t)mm_info->dma_addr,
				(uintptr_t)mm_info->phys_addr,
				(uintptr_t)(ptr_mm_cma->share_buf.ref_cnt));
		if (!ptr_mm_cma->share_buf.ref_cnt) {
...
static int adlak_malloc_share_through_dma(struct adlak_mem *mm,
					  struct adlak_mem_handle *mm_info)
{
	int ret;
	// gfp_t gfp = 0;
	if (!ptr_mm_cma->share_buf.share_buf_cpu_addr) {
		mm_info->req.bytes =
			(size_t)ptr_mm_cma->share_buf.share_buf_size;
	...
	mm_info->iova_addr = mm_info->phys_addr;
	ptr_mm_cma->share_buf.ref_cnt++;
	adlak_os_printf("%s: dma_addr= 0x%lX,  phys_addr= 0x%lX,size=%lu KByte, ref_cnt=%lu\n",
			__func__, (uintptr_t)mm_info->dma_addr,
			(uintptr_t)mm_info->phys_addr,
...
static void adlak_cma_claim_dev(void)
{
...
static void adlak_cma_release_dev(void)
{
	AML_LOG_DEBUG("%s[+]", __func__);
	...
	if (!(pbuf_req->mem_type & ADLAK_ENUM_MEMTYPE_INNER)) {
		...
		if (ptr_mm_cma->share_buf.share_swap_en) {
			if (pbuf_req->mem_type & ADLAK_ENUM_MEMTYPE_SHARE) {
				/* share memory between different model only
				 * supports soc without smmu for the time being
				 */
				mm_info->req.mem_type =
					mm_info->req.mem_type |
					ADLAK_ENUM_MEMTYPE_INNER_SHARE;
			}
		}
	}
...
static int
adlak_cma_cache_flush(struct adlak_mem_handle *mm_info,
		      struct adlak_sync_cache_ext_info *sync_cache_ext_info)
{
	...
	if (mm_info->mem_src == ADLAK_ENUM_MEMSRC_OS ||
		...
		adlak_os_flush_cache(&ptr_mm_cma->mm, mm_info,
				     sync_cache_ext_info);
	}
	return 0;
}
...
static int
adlak_cma_cache_invalid(struct adlak_mem_handle *mm_info,
			struct adlak_sync_cache_ext_info *sync_cache_ext_info)
{
	if (ADLAK_IS_ERR_OR_NULL(mm_info))
		return -1;
	...
	if (mm_info->mem_src == ADLAK_ENUM_MEMSRC_OS ||
	    ...
	    mm_info->mem_src == ADLAK_ENUM_MEMSRC_RESERVED ||
	    mm_info->mem_src == ADLAK_ENUM_MEMSRC_EXT_PHYS) {
		adlak_os_invalid_cache(&ptr_mm_cma->mm, mm_info,
				       sync_cache_ext_info);
	}
	return 0;
}
...
adlak_cma_alloc(struct adlak_context_smmu_attr *ctx_smmu_attr,
		struct adlak_buf_req *pbuf_req)
{
	...
	if (ptr_mm_cma->has_mem_pool) {
		...
		if (ERR(NONE) != ret) {
			...
			ret = adlak_malloc_through_dma(&ptr_mm_cma->mm,
						       mm_info);
		} else {
			goto end;
		}
		...
		if (mm_info->req.mem_type & ADLAK_ENUM_MEMTYPE_INNER_SHARE) {
			ret = adlak_malloc_share_through_dma(&ptr_mm_cma->mm,
							     mm_info);
			goto end;
		} else {
			// alloc memory through dma api
	...
	if (ret) {
		...
		if (!ctx_smmu_attr) {
			...
			if (0 == (mm_info->mem_type &
				  ADLAK_ENUM_MEMTYPE_INNER_SHARE)) {
				ptr_mm_cma->usage.alloced_umd +=
					mm_info->req.bytes;
				ctx_smmu_attr->alloc_byte += mm_info->req.bytes;
			}
		}
...
static struct adlak_mem_handle *
adlak_cma_attach(struct adlak_context_smmu_attr *ctx_smmu_attr,
		 struct adlak_extern_buf_info *pbuf_req)
{
	...
	if (!size ||
	    !ADLAK_IS_ALIGNED((unsigned long)(size), ADLAK_PAGE_SIZE)) {
		goto early_exit;
	}
...
static int adlak_mem_cma_init(void)
{
	...
	if (adlak_remap_region_nocache(ptr_mm_cma->padlak, &ptr_mm_cma->mem_pool)) {
		ret = -1;
		goto err;
	}
#else
...
#error "Not support CMA in freertos.Please change with reserved_memory or others"
	...
	if (adlak_alloc_cma_region_nocache()) {
		ret = -1;
		goto err;
	}
#endif
	...
	if (ptr_mm_cma->has_mem_pool) {
		...
		if (adlak_mem_pools_mgr_init(&exclude_area))
			goto err;
	}
...
void adlak_mem_cma_unregister(struct adlak_device *padlak,
...
int adlak_mem_cma_register(struct adlak_device *padlak,
			   struct adlak_mem_operator *ops)
{
	int ret = 0;
	...
	if (!ptr_mm_cma) {
		ptr_mm_cma = adlak_os_zalloc(sizeof(struct adlak_mem_cma),
					     ADLAK_GFP_KERNEL);
