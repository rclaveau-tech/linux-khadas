...
int adlak_smmu_tlb_alloc(struct adlak_device *padlak)
{
	...
	for (idx1 = 0; idx1 < SMMU_TLB1_ENTRY_COUNT; idx1++) {
		psmmu->tlb_l2[idx1].size =
			ADLAK_ALIGN(SMMU_TLB2_SIZE, SMMU_ALIGN_SIZE);
		buf_req.mem_type = ADLAK_ENUM_MEMTYPE_CONTIGUOUS |
				   ADLAK_ENUM_MEMTYPE_INNER;
	...
	AML_LOG_INFO("tlb1 section size=%lu Bytes.",
		     (uintptr_t)psmmu->tlb_l1.size);
	AML_LOG_INFO("tlb2 per-section size=%lu KBytes.",
		     (uintptr_t)psmmu->tlb_l2[0].size / 1024);
	AML_LOG_INFO("tlb3 per-section size=%lu KBytes.",
		     (uintptr_t)psmmu->tlb_l3[0][0].size / 1024);
...
int adlak_smmu_tlb_fill_init(struct __adlak_smmu *psmmu)
{
	...
	/*fill tlb level 1*/
	tlb_logic = (uintptr_t)adlak_mm_vmap(psmmu->tlb_l1.mm_info);
	AML_LOG_DEBUG("l1 cpu_addr= %lX,mem_type=%u",
		      (uintptr_t)adlak_mm_vmap(psmmu->tlb_l1.mm_info),
	...
	for (idx1 = 0; idx1 < SMMU_TLB1_ENTRY_COUNT; idx1++) {
		tlb_logic =
			(uintptr_t)adlak_mm_vmap(psmmu->tlb_l2[idx1].mm_info);
		for (idx2 = 0; idx2 < SMMU_TLB2_ENTRY_COUNT_2M; idx2++) {
			entry_val =
				psmmu->tlb_l3[idx1][idx2].mm_info->phys_addr;
	...
	for (idx1 = 0; idx1 < SMMU_TLB1_ENTRY_COUNT; idx1++) {
		for (idx2 = 0; idx2 < SMMU_TLB2_ENTRY_COUNT_2M; idx2++) {
			tlb_logic = (u64)(uintptr_t)adlak_mm_vmap
				    (psmmu->tlb_l3[idx1][idx2].mm_info);
...
int adlak_smmu_tlb_dump(struct __adlak_smmu *psmmu)
{
	...
	adlak_os_printf("phys_addr: 0x%08X\n",
			(uintptr_t)psmmu->tlb_l1.mm_info->phys_addr);
	tlb_logic = (uintptr_t)adlak_mm_vmap(psmmu->tlb_l1.mm_info);
	for (idx1 = 0; idx1 < SMMU_TLB1_ENTRY_COUNT; idx1++) {
#ifdef CONFIG_64BIT
		adlak_os_printf("offset:0x%08X\t0x%llX\n",
				(u32)(idx1 * sizeof(u64)),
				_read_page_entry(tlb_logic +
						 (idx1 * sizeof(u64))));
#else
	...
	for (idx1 = 0; idx1 < 1; idx1++) {
		...
		adlak_os_printf("phys_addr: 0x%08X\n",
				(uintptr_t)psmmu->tlb_l2[idx1].mm_info->phys_addr);
		tlb_logic = (uintptr_t)adlak_mm_vmap(psmmu->tlb_l2[idx1].mm_info);
...
#else
	adlak_os_printf("offset:0x%08X\t0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X\n",
			(u32)(idx2 * sizeof(u64)),
			_read_page_entry(tlb_logic + (idx2 * sizeof(u32))),
	...
	for (idx1 = 0; idx1 < SMMU_TLB1_ENTRY_COUNT; idx1++) {
		...
		for (idx2 = 0; idx2 < SMMU_TLB2_ENTRY_COUNT_2M; idx2++) {
			...
			adlak_os_printf("phys_addr: 0x%08X\n",
					(uintptr_t)psmmu->tlb_l3[idx1][idx2]
						.mm_info->phys_addr);
			tlb_logic = (uintptr_t)adlak_mm_vmap(psmmu->tlb_l3[idx1][idx2].mm_info);
	...
	adlak_os_printf("offset:0x%08X\t0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X\n",
			(u32)(idx3 * sizeof(u64)),
			_read_page_entry(tlb_logic + (idx3 * sizeof(u32))),
...
int adlak_smmu_tlb_add(struct __adlak_smmu *psmmu, dma_addr_t iova_addr,
		       phys_addr_t phys_addr, size_t size)
{
	...
	tlb_logic = (uintptr_t)(adlak_mm_vmap(psmmu->tlb_l3[tlb_l1_offset][tlb_l2_offset].mm_info));
	entry_val = phys_addr;
...
static int adlak_smmu_tlb_init(struct adlak_mem *mm)
{
	...
	padlak->psmmu =
		adlak_os_zalloc(sizeof(struct __adlak_smmu), ADLAK_GFP_KERNEL);
	if (!padlak->psmmu)
		return ERR(ENOMEM);
...
static int adlak_smmu_tlb_flush_cache(struct adlak_mem *mm)
{
	...
	adlak_flush_cache(padlak, psmmu->tlb_l1.mm_info, &sync_cache_extern);
