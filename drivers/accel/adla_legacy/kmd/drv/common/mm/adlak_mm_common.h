...
#endif
#define ADLAK_MM_POOL_PAGE_SIZE BIT(ADLAK_MM_POOL_PAGE_SHIFT)
...
enum adlak_mem_type_inner {
	...
	ADLAK_ENUM_MEMTYPE_INNER_PA_WITHIN_4G =
		(1u << 4), // physical address less than 4Gbytes
...
struct adlak_mem_handle {
	...
	u32 mem_type; // the mem_type may not same as request info
	u32 from_pool;
	u64 uid;
};
...
struct adlak_mem_operator {
	...
	void (*free)(struct adlak_context_smmu_attr *ctx_smmu_attr,
	...
	int (*cache_invalid)(struct adlak_mem_handle *mm_info,
			     struct adlak_sync_cache_ext_info *sync_cache_ext_info);
	void (*smmu_init)(void *ptr, u8 type);
	void (*smmu_deinit)(void *ptr, u8 type);
	void (*smmu_tlb_invalidate)(struct adlak_context_smmu_attr *ctx_smmu_attr);
	void (*get_usage)(struct adlak_mem_usage *usage);
	u64 (*get_smmu_entry)(struct adlak_context_smmu_attr *ctx_smmu_attr, u8 type);
...
unsigned long adlak_alloc_from_bitmap_pool(struct adlak_mm_pool_priv *pool,
					   size_t size);
unsigned long
adlak_alloc_from_bitmap_pool_reverse(struct adlak_mm_pool_priv *pool,
				     size_t size);

void adlak_free_to_bitmap_pool(struct adlak_mm_pool_priv *pool,
			       dma_addr_t start, size_t size);

void adlak_set_bitmap_pool(struct adlak_mm_pool_priv *pool, dma_addr_t start,
			   size_t size);
