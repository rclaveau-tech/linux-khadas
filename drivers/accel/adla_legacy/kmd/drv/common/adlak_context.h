...
struct adlak_context {
	...
	struct adlak_os_mutex_t *context_mutex;
	uintptr_t wait; // wait_queue_head_t *
	u32 state;
	u32 invoke_cnt;

	u32 invoke_time_elapsed_total;
	u32 invoke_time_elapsed_tmp;
	s64 macc_count;

	struct adlak_os_sema_t *invoke_state;

	struct adlak_os_sema_t *sem_irq;

#define CONTEXT_STATE_USED BIT(2)
#define CONTEXT_STATE_INITED BIT(1)
#define CONTEXT_STATE_CLOSED BIT(0)
	s32 net_id;
	size_t mem_alloced;
	...
	struct adlak_os_sema_t *ctx_idle;
};
 ...
 * @return int
 */
int adlak_destroy_context(struct adlak_device *padlak,
 ...
 */
struct context_buf *find_buffer_by_desc(struct adlak_context *context,
					void *pmm_info);
