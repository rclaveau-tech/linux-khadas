...
static int adlak_cmq_dump(struct adlak_task *ptask)
{
	...
	struct adlak_cmq_buffer *cmq_buf_info =
		ptask->context->pmodel_attr->cmq_buffer; // TODO
	adlak_os_printf("Dump cmq buffer:");
	pcmq_buf = adlak_mem_vmap(cmq_buf_info->cmq_mm_info);
	if (ADLAK_CMQ_BUFFER_TYPE_PRIVATE ==
	    ptask->context->pmodel_attr->cmq_buffer_type) {
		start = 0;
		end = cmq_buf_info->size / sizeof(u32);
	} else {
		start = ptask->cmd_offset_start / sizeof(u32);
		end = ptask->cmd_offset_end / sizeof(u32);
	}

	adlak_os_printf("cmd_offset_start: 0x%08X\t;cmd_offset_end: 0x%08X\n",
			start * sizeof(u32), end * sizeof(u32));
	if (start < end) {
		while (start < end) {
			adlak_os_printf("offset:0x%08X\t0x%08X 0x%08X 0x%08X 0x%08X\n",
					(u32)(start * sizeof(u32)), pcmq_buf[start + 0],
					pcmq_buf[start + 1], pcmq_buf[start + 2],
					pcmq_buf[start + 3]);
			start += 4;
		}
		...
		// start >= end;
		while (start < (padlak->cmq_buffer_public.size / sizeof(u32))) {
			adlak_os_printf("offset:0x%08X\t0x%08X 0x%08X 0x%08X 0x%08X\n",
					(u32)(start * sizeof(u32)), pcmq_buf[start + 0],
					pcmq_buf[start + 1], pcmq_buf[start + 2],
					pcmq_buf[start + 3]);
...
static void adlak_debug_buf_dump(void *cpu_addr, int size)
{
	...
	u32 start, i, cmq_size_dw;
	u32 *pcmq_buf = NULL;

	AML_LOG_DEBUG("Dump  buffer:");
	pcmq_buf = cpu_addr;
	cmq_size_dw = size / sizeof(u32);
	start = 0;
	i = 0;
	for (i = 0; i < cmq_size_dw;) {
		AML_LOG_DEFAULT("offset:0x%08X\t0x%08X 0x%08X 0x%08X 0x%08X\n",
				(u32)((start + i) * sizeof(u32)),
				pcmq_buf[start + i], pcmq_buf[start + i + 1],
				pcmq_buf[start + i + 2],
				pcmq_buf[start + i + 3]);
		i = i + 4;
	}
...
static inline u32 adlak_cmq_cal_tail(struct adlak_circular_buffer *buffer,
				     u32 tail, u32 inc_size)
{
	return (tail + inc_size) % buffer->size; // Move tail circularly
}

static u32 adlak_cmq_get_cur_tail(struct adlak_circular_buffer *buffer)
{
	return buffer->tail;
}

static void adlak_cmq_write_data(struct adlak_circular_buffer *buffer,
				 u32 value)
{
	...
	buffer->tail =
		(buffer->tail + 1) % buffer->size; // Move tail circularly

	if (buffer->tail == buffer->head) {
		buffer->head = (buffer->head + 1) %
			       buffer->size; // Move head circularly
	}
...
static void adla_cmq_modify_data(struct adlak_circular_buffer *buffer,
...
static void adlak_cmq_write_buffer(struct adlak_circular_buffer *buffer,
				   const u32 *source, u32 length)
{
	u32 space_to_end =
		buffer->size -
		buffer->tail; // Calculate available space until the end of the buffer

	AML_LOG_DEBUG("wr buffer start tail=%u", buffer->tail);

	if (length <= space_to_end) {
		memcpy(buffer->data + buffer->tail, source,
		       length * sizeof(u32));
		buffer->tail = (buffer->tail + length) %
			       buffer->size; // Move tail circularly
	} else {
		memcpy(buffer->data + buffer->tail, source,
		       space_to_end * sizeof(u32));
		memcpy(buffer->data, source + space_to_end,
		       (length - space_to_end) * sizeof(u32));
		buffer->tail = (buffer->tail + length) %
			       buffer->size; // Move tail circularly
		buffer->head =
			buffer->tail; // Move head to tail position (overwrite old data)
	}
...
static void
adlak_cmq_merge_modify_data(const struct adlak_submit_task *submit_task,
			    struct adlak_circular_buffer *buffer,
			    u32 cmq_cfg_index, const u32 *modify_buf,
			    u32 modify_size)
{
	struct adlak_circular_buffer buffer_inner;
	u32 cmd_bypass_cnt, cmd_modify_cnt;
	...
	while (modify_size) {
		...
		modify_buf += 1;
		buffer_inner.tail = adlak_cmq_cal_tail(&buffer_inner, buffer_inner.tail,
						       cmd_bypass_cnt);
		if (cmd_modify_cnt) {
			adlak_cmq_write_buffer(&buffer_inner,
					       (const u32 *)modify_buf,
					       cmd_modify_cnt);
		}
...
u32 g_adlak_emu_dev_wpt;
u32 g_adlak_emu_dev_rpt;
u32 g_adlak_emu_dev_cmq_total_size;
...
u32 adlak_emu_hal_get_ps_rpt(void *data)
{
	u32 wpt_u32, rpt;

	if (g_adlak_emu_dev_wpt > 16)
		wpt_u32 = (g_adlak_emu_dev_wpt - 16) / sizeof(u32);
	else
		wpt_u32 = 0;
...
static s32 adlak_get_max_outstanding_outputs(enum ADLAK_PLATFORM_MODULE module)
{
	switch (module) {
...
static u32 adlak_gen_dep_cmd(int dependency_mode,
			     const struct adlak_submit_task *task,
			     struct adlak_submit_dep_fixup *pdep_fixups_base,
			     s32 pwe_flid_offset, s32 pwx_flid_offset,
			     s32 rs_flid_offset, s32 start_id_pwe,
			     s32 start_id_pwx, s32 start_id_rs)
{
	u32 cmd = PS_CMD_SET_DEPENDENCY; // default no dependency
	struct adlak_submit_dep_fixup *dep = NULL;

	{
		bool has_pwe_dependency = false, has_pwx_dependency = false,
		     has_rs_dependency = false;
		s32 pwe_dlid = -1, pwx_dlid = -1, rs_dlid = -1;
		int i;
		u32 dependency;
		...
		if (dependency_mode != ADLAK_DEPENDENCY_MODE_PARSER)
			return cmd;
#if ADLAK_DEBUG_CMQ_PATTTCHING_EN
		AML_LOG_DEBUG("pwe_flid_offset=%d,pwx_flid_offset=%d,rs_flid_offset=%d",
			      pwe_flid_offset, pwx_flid_offset, rs_flid_offset);
		AML_LOG_DEBUG("global_id_pwe=%d,global_id_pwx=%d,global_id_rs=%d",
			      start_id_pwe, start_id_pwx, start_id_rs);
		AML_LOG_DEBUG("dep_info_offset=%d,dep_info_count=%d",
			      task->dep_info_offset, task->dep_info_count);
		AML_LOG_DEBUG("reg_fixup_offset=%d,reg_fixup_count=%d",
			      task->reg_fixup_offset, task->reg_fixup_count);
#endif
...
#if ADLAK_DEBUG_CMQ_PATTTCHING_EN
		AML_LOG_DEBUG("pwe_dlid =%ld, pwx_dlid =%ld,rs_dlid=%ld",
			      pwe_dlid, pwx_dlid, rs_dlid);
#endif
		if (pwe_dlid >= 0) {
			if (pwe_dlid >= pwe_flid_offset) {
#if ADLAK_DEBUG_CMQ_PATTTCHING_EN
				AML_LOG_DEBUG
					("pwe_dlid =%ld, pwe_flid_offset =%d,start_pwe_flid=%d",
					 pwe_dlid, pwe_flid_offset,
					 task->start_pwe_flid);
#endif
				has_pwe_dependency = adlak_has_dependency
					(pwe_dlid, task->start_pwe_flid,
					 adlak_get_max_outstanding_outputs
						(ADLAK_PLATFORM_MODULE_PWE));
				pwe_dlid = start_id_pwe +
					   (pwe_dlid - pwe_flid_offset) + 1;
			}
		}
		if (pwx_dlid >= 0) {
...
#if ADLAK_DEBUG_CMQ_PATTTCHING_EN
				AML_LOG_DEBUG
					("pwx_dlid =%ld, pwx_flid_offset =%d,start_pwx_flid=%d",
					 pwx_dlid, pwx_flid_offset,
					 task->start_pwx_flid);
#endif
				has_pwx_dependency = adlak_has_dependency
					(pwx_dlid, task->start_pwx_flid,
					 adlak_get_max_outstanding_outputs
						(ADLAK_PLATFORM_MODULE_PWX));
				pwx_dlid = start_id_pwx +
					   (pwx_dlid - pwx_flid_offset) + 1;
			}
		}
		...
		if (rs_dlid >= 0) {
			if (rs_dlid >= rs_flid_offset) {
				...
				AML_LOG_DEBUG("rs_dlid =%ld, rs_flid_offset =%d,start_rs_flid=%d\n",
					      rs_dlid, rs_flid_offset,
					      task->start_rs_flid);
#endif
				has_rs_dependency = adlak_has_dependency
	(rs_dlid, task->start_rs_flid, adlak_get_max_outstanding_outputs(ADLAK_PLATFORM_MODULE_RS));
				rs_dlid = start_id_rs + (rs_dlid - rs_flid_offset) + 1;
			}
		}

		dependency =
			(has_rs_dependency ? PS_CMD_DEPENDENCY_RS_VALID_MASK :
					     0) |
			(has_pwe_dependency ? PS_CMD_DEPENDENCY_PWE_VALID_MASK :
					      0) |
			(has_pwx_dependency ? PS_CMD_DEPENDENCY_PWX_VALID_MASK :
					      0) |
			((rs_dlid << PS_CMD_DEPENDENCY_RS_ID_SHIFT) &
			 PS_CMD_DEPENDENCY_RS_ID_MASK) |
			((pwe_dlid << PS_CMD_DEPENDENCY_PWE_ID_SHIFT) &
			 PS_CMD_DEPENDENCY_PWE_ID_MASK) |
			((pwx_dlid << PS_CMD_DEPENDENCY_PWX_ID_SHIFT) &
			 PS_CMD_DEPENDENCY_PWX_ID_MASK);
...
size_required, size_remain, rpt_u8, wpt_u8, cmq_buf_info->size);
		...
		if (retry > 3) {
			// You can't sleep here,and you can't wait too long here, otherwise
			adlak_os_udelay(1);
...
static void
adlak_update_addr_fixups(const struct adlak_submit_task *task,
			 ...
			 u32 cmq_offset_cfg)
{
	...
	for (i = 0; i < task->addr_fixup_count; ++i) {
		...
		cmq_offset =
			adlak_cmq_cal_tail(buffer, cmq_offset_cfg,
					   (addr_fixup->loc / sizeof(u32)));
		buffer->data[cmq_offset] =
			(buffer->data[cmq_offset] & (~addr_fixup->mask)) |
			((((uintptr_t)addr_fixup->addr / addr_fixup->unit)
			  << addr_fixup->shift) &
			 addr_fixup->mask);
...
static void adlak_update_reg_fixups
(int dependency_mode, const struct adlak_submit_task *task,
struct adlak_submit_reg_fixup *preg_fixups_base,
struct adlak_circular_buffer *buffer, u32 cmq_offset_cfg)
{
	...
	for (i = 0; i < task->reg_fixup_count; ++i) {
		...
		if (ADLAK_REG_FIXUP_TYPE_PW_COMP_FLUSH_MODE ==
		    reg_fixup->type) {
			mode = reg_fixup->modes[dependency_mode];
			cmq_offset = adlak_cmq_cal_tail(buffer, cmq_offset_cfg,
							reg_fixup->loc / sizeof(u32));
...
static void adlak_update_module_dependency
	(int dependency_mode, const struct adlak_submit_task *task,
	 struct adlak_submit_dep_fixup *pdep_fixups_base, s32 pwe_flid_offset,
	 s32 pwx_flid_offset, s32 rs_flid_offset, s32 start_id_pwe,
	 s32 start_id_pwx, s32 start_id_rs, struct adlak_circular_buffer *buffer,
	 u32 cmq_offset_cfg)
{
	struct adlak_submit_dep_fixup *dep = NULL;
	s32 invoke_start_flid, dlid = -1;
	s32 i, task_start_flid, flid_offset, max_outstanding_outputs;
	u32 mode, cmq_offset;
	s32 pwe_dlid = -1, pwx_dlid = -1, rs_dlid = -1;
	...
	for (i = 0; i < task->dep_info_count; ++i) {
		...
		if (dlid >= flid_offset) {
			if (adlak_has_dependency(dlid, task_start_flid,
						 max_outstanding_outputs)) {
				dlid = invoke_start_flid +
				       (dlid - flid_offset) + 1;
				mode = dep->dep_modes[dependency_mode];
				cmq_offset = adlak_cmq_cal_tail(buffer, cmq_offset_cfg,
								dep->id_loc / sizeof(u32));
				buffer->data[cmq_offset] =
					(buffer->data[cmq_offset] &
					 (~dep->id_mask)) |
					((dlid << dep->id_shift) &
...
int adlak_queue_schedule_update(struct adlak_device *padlak,
				struct adlak_task **ptask_sch_cur,
				s32 net_id_pre)
{
	...
	if (list_empty(&pwq->pending_list)) {
		AML_LOG_WARN("pending_list is empty!,pwq->pending_num=%d",
			     pwq->pending_num);
		*ptask_sch_cur = NULL;
		ret = -1;
		pwq->pending_num--;
	...
	if (net_id_pre < 0) {
		ptask = list_first_entry(&pwq->pending_list,
					 typeof(struct adlak_task), head);
		if (ptask)
			ret = 0;
		else
			ret = -1;
		...
		ret = -1;
		list_for_each_entry_safe(ptask, ptask_tmp, &pwq->pending_list,
					 head) {
...
static struct adlak_model_attr *
adlak_model_create(struct adlak_context *context,
		   struct adlak_network_desc *psubmit_desc)
{
	...
	pmodel_attr = adlak_os_zalloc(sizeof(struct adlak_model_attr),
				      ADLAK_GFP_KERNEL);
	if (!pmodel_attr)
...
(sizeof(struct adlak_submit_task) * psubmit_desc->tasks_num, ADLAK_GFP_KERNEL);
	...
	if (psubmit_desc->config_total_size) {
		pmodel_attr->config = adlak_os_vmalloc(sizeof(u8) * psubmit_desc->config_total_size,
						       ADLAK_GFP_KERNEL);
	...
	if (psubmit_desc->dep_fixups_num) {
		pmodel_attr->submit_dep_fixups =
			adlak_os_vmalloc(sizeof(struct adlak_submit_dep_fixup) *
						 psubmit_desc->dep_fixups_num,
					 ADLAK_GFP_KERNEL);
	...
	if (psubmit_desc->reg_fixups_num) {
		pmodel_attr->submit_reg_fixups =
			adlak_os_vmalloc(sizeof(struct adlak_submit_reg_fixup) *
						 psubmit_desc->reg_fixups_num,
					 ADLAK_GFP_KERNEL);
	...
	if (psubmit_desc->cmd_buf_attr.support) {
		/*check cmq_buffer*/
		target_buf = find_buffer_by_desc(context,
						 (void *)(uintptr_t)(psubmit_desc->cmd_buf_attr
								     .mem_handle));
		if (!target_buf) {
		...
		pmodel_attr->cmd_buf_attr.support =
			psubmit_desc->cmd_buf_attr.support;
	...
	ret = copy_from_user((void *)pmodel_attr->config,
			     ...
			     sizeof(u8) * psubmit_desc->config_total_size);
	if (ret) {
	...
	ret = copy_from_user((void *)pmodel_attr->submit_tasks,
			     ...
			     sizeof(struct adlak_submit_task) *
				     psubmit_desc->tasks_num);
	if (ret) {
		AML_LOG_ERR("copy from user failed!");
		ret = ERR(EFAULT);
		goto err_copy_from_user;
	...
	if (pmodel_attr->submit_dep_fixups) {
		ret = copy_from_user((void *)pmodel_attr->submit_dep_fixups,
				     ...
				     sizeof(struct adlak_submit_dep_fixup) *
					   psubmit_desc->dep_fixups_num);
		if (ret) {
			AML_LOG_ERR("copy from user failed!");
			ret = ERR(EFAULT);
	...
	if (pmodel_attr->submit_reg_fixups) {
		ret = copy_from_user((void *)pmodel_attr->submit_reg_fixups,
				     ...
				     sizeof(struct adlak_submit_reg_fixup) *
					   psubmit_desc->reg_fixups_num);
		if (ret) {
			AML_LOG_ERR("copy from user failed!");
			ret = ERR(EFAULT);
	...
	pmodel_attr->submit_dep_fixups =
		(void *)(uintptr_t)psubmit_desc->dep_fixups_va;
	if (psubmit_desc->reg_fixups_num) {
		pmodel_attr->submit_reg_fixups =
			(void *)(uintptr_t)psubmit_desc->reg_fixups_va;
	} else {
		pmodel_attr->submit_reg_fixups = NULL;
	}
#endif
...
static void adlak_mark_the_last_hw_layer(struct adlak_model_attr *pmodel_attr)
{
	...
	for (task_idx = pmodel_attr->submit_tasks_num - 1; task_idx >= 0;
	     task_idx--) {
		psubmitask = psubmitask_base + task_idx;
		if (0 >= (psubmitask->config_size +
			  psubmitask->config_v2.common_size)) {
			// software operations
			continue;
		}
...
static int adlak_invoke_pre_check(struct adlak_model_attr *pmodel_attr)
{
	...
	u32 task_idx;
	u32 size_max, size_per_task;
	u32 cmq_size_expected = 0;
	struct adlak_submit_task *psubmitask_base = NULL, *psubmitask = NULL;

	psubmitask_base = pmodel_attr->submit_tasks;
	size_max = 0;

	for (task_idx = 0; task_idx < pmodel_attr->submit_tasks_num;
	     task_idx++) {
		psubmitask = psubmitask_base + task_idx;
		ASSERT(!((psubmitask->config_size) > 0 &&
			 (psubmitask->config_v2.common_size > 0)));
		if (0 >= (psubmitask->config_size +
			  psubmitask->config_v2.common_size)) {
			// software operations
			continue;
		}
		if (pmodel_attr->hw_layer_first == -1)
			pmodel_attr->hw_layer_first = task_idx;

		size_per_task = ((u32)ADLAK_PS_CMD_EXTERN_LEN) * sizeof(u32);
...
static struct adlak_task *
adlak_invoke_create(struct adlak_context *context,
		    struct adlak_network_invoke_desc *pinvoke_desc)
{
	...
	if (pinvoke_desc->start_idx < 0 ||
	    pinvoke_desc->end_idx >= pmodel_attr->submit_tasks_num ||
	    pinvoke_desc->start_idx > pinvoke_desc->end_idx) {
		AML_LOG_ERR("invoke id is invalid!");
		return ADLAK_ERR_PTR(ERR(EINVAL));
	}
	if (!pmodel_attr->invoke_attr_rsv) {
		pinvoke_attr = adlak_os_zalloc(sizeof(struct adlak_task),
					       ADLAK_GFP_KERNEL);
		if (!pinvoke_attr) {
	...
	if (pinvoke_desc->addr_fixups_num) {
#ifndef CONFIG_ADLA_FREERTOS
		if (pmodel_attr->submit_addr_fixups && pmodel_attr->addr_fixups_num !=
						       pinvoke_desc->addr_fixups_num) {
			adlak_os_free(pmodel_attr->submit_addr_fixups);
			pmodel_attr->submit_addr_fixups = NULL;
		}
		...
		ret = copy_from_user((void *)pmodel_attr->submit_addr_fixups,
				     ...
				     sizeof(struct adlak_submit_addr_fixup) *
					   pmodel_attr->addr_fixups_num);
		if (ret) {
			AML_LOG_ERR("copy from user failed!");
		...
		pmodel_attr->addr_fixups_num = pinvoke_desc->addr_fixups_num;
		pmodel_attr->submit_addr_fixups =
			(void *)(uintptr_t)pinvoke_desc->addr_fixups_va;
	...
	pinvoke_desc->invoke_register_idx =
		pinvoke_attr->invoke_idx; // return invoke index
...
static int adlak_invoke_del_from_nosch_list(struct list_head *hd, s32 net_id,
					    s32 invoke_id)
{
	int ret = 0;
	struct adlak_task *ptask = NULL, *ptask_tmp = NULL;
	...
	if (!list_empty(hd)) {
		list_for_each_entry_safe(ptask, ptask_tmp, hd, head) {
			if (ptask) {
				if (net_id != -1 &&
				    net_id != ptask->context->net_id) {
					continue;
				}
				if (invoke_id != -1 &&
				    invoke_id != ptask->invoke_idx) {
					continue;
				}
				ret++;
				list_del(&ptask->head);
...
// return deleted count
static int adlak_invoke_del_from_sch_list(struct list_head *hd, s32 net_id,
					  s32 invoke_id)
{
	int ret = 0;
	struct adlak_task *ptask = NULL, *ptask_tmp = NULL;
...
static int adlak_invoke_del_with_invokeid(struct adlak_device *padlak,
					  s32 net_id, s32 invoke_id)
{
	/*
	 * - if in pendding list,
	 * - mutex lock;
	...
	ret = adlak_invoke_del_from_nosch_list(&pwq->finished_list, net_id,
					       invoke_id);
	if (ret > 0) {
		ret = 0;
		if (invoke_id >= 0)
	...
	ret = adlak_invoke_del_from_nosch_list(&pwq->pending_list, net_id,
					       invoke_id);
	if (ret > 0) {
		ret = 0;
	...
	ret = adlak_invoke_del_from_sch_list(&pwq->scheduled_list, net_id,
					     invoke_id);
	if (ret > 0) {
		ret = -1;
		//net_is_used = true;
...
int adlak_invoke_del_all(struct adlak_device *padlak, s32 net_id)
{
			    ...
			    struct adlak_network_desc *psubmit_desc)
{
	...
	pmodel_attr->hw_layer_last_in_first_smmu_table =
		psubmit_desc->hw_last_layer_in_first_smmu;
	...
	if (pmodel_attr->cmq_buffer_type == ADLAK_CMQ_BUFFER_TYPE_PUBLIC)
	...
	if (pmodel_attr->cmq_buffer_type == ADLAK_CMQ_BUFFER_TYPE_PUBLIC) {
		cmq_buf_info =
			(struct adlak_cmq_buffer *)pmodel_attr->cmq_buffer;
		if (pmodel_attr->size_max_in_layer > cmq_buf_info->size) {
			ret = -1;
			AML_LOG_ERR
("the maximum size limit is exceeded which the cmq need (%d),but buffer size max(%d);"
"\nPlease increase the size of command queue when insmod.",
pmodel_attr->size_max_in_layer,
cmq_buf_info->size);
...
static int
adlak_invoke_add_queue(struct adlak_context *context,
		       struct adlak_network_invoke_desc *pinvoke_desc)
{
	int ret = 0;
	struct adlak_device *padlak = context->padlak;
...
static void adlak_net_pre_progress_part1(struct adlak_context *context)
{
	struct adlak_submit_task *psubmitask_base = NULL, *psubmitask = NULL;
	struct adlak_model_attr *pmodel_attr = NULL;
	u32 task_idx, module_index, active_modules, output_modules,
		fence_modules;
	const u32 parser_active_modules[ADLAK_PLATFORM_MODULE_COUNT] = {
	...
	for (task_idx = 0; task_idx < pmodel_attr->submit_tasks_num;
		...
		active_modules = 0;
		for (module_index = 0;
		     module_index < ADLAK_PLATFORM_MODULE_COUNT;
		     ++module_index) {
			if (psubmitask->active_modules & (1 << module_index)) {
				active_modules |=
					parser_active_modules[module_index];
			}
		}
		output_modules = 0;
		if (psubmitask->output_modules &
		    (1 << ADLAK_PLATFORM_MODULE_PWE)) {
			output_modules |= PS_CMD_EXECUTE_OUTPUT_PWE_MASK;
		}

		if (psubmitask->output_modules &
		    (1 << ADLAK_PLATFORM_MODULE_PWX)) {
			output_modules |= PS_CMD_EXECUTE_OUTPUT_PWX_MASK;
		}

		if (psubmitask->output_modules &
		    (1 << ADLAK_PLATFORM_MODULE_RS)) {
...
#define ADLAK_VER_PARSR_V2 (0x0400)
	...
	//config_base = (u8 *)((uintptr_t)adlak_mem_vmap(pmodel_attr->cmd_buf_attr.mm_info));

	psubmitask_base = pmodel_attr->submit_tasks;
	for (task_idx = 0; task_idx < pmodel_attr->submit_tasks_num;
	     task_idx++) {
		psubmitask = psubmitask_base + task_idx;
		if (psubmitask->config_v2.common_size <= 0) {
			// software operations
			continue;
		}
		/* modify_buf  = (u32 *)(config_base + psubmitask->config_v2.modify_offset);
				 ...
				 struct adlak_network_del_desc *submit_del)
{
	...
	ret = adlak_invoke_del_with_invokeid(padlak,
					     submit_del->net_register_idx, -1);
	if (!ret)
...
int adlak_get_status_request(struct adlak_context *context,
			     struct adlak_get_stat_desc *stat_desc)
{
	...
	if (!list_empty(hd)) {
		list_for_each_entry_safe(ptask, ptask_tmp, hd, head) {
			if (ptask) {
				if (stat_desc->net_register_idx !=
				    ptask->context->net_id) {
					continue;
				}
				if (stat_desc->invoke_register_idx !=
				    ptask->invoke_idx) {
					continue;
				}
				stat_desc->profile_en =
					ptask->profilling.profile_en;
				stat_desc->invoke_time_us =
					ptask->profilling.time_elapsed_us;
				stat_desc->start_idx = ptask->invoke_start_idx;
				stat_desc->end_idx = ptask->invoke_end_idx;
				stat_desc->profile_rpt = 0;
				stat_desc->axi_freq_cur =
					padlak->clk_axi_freq_real;
				stat_desc->core_freq_cur =
					padlak->clk_core_freq_real;
				stat_desc->mem_alloced_umd =
					context->smmu_attr.alloc_byte;

				adlak_mem_get_usage(&mem_usage);
				stat_desc->mem_alloced_base =
					mem_usage.alloced_kmd;
				stat_desc->mem_pool_size = mem_usage.pool_size;
				stat_desc->mem_pool_used =
					mem_usage.alloced_kmd +
					mem_usage.alloced_umd +
					mem_usage.share_buf_size;
				stat_desc->efficiency =
					adlak_dmp_get_efficiency(padlak);
				if (ADLAK_SUBMIT_STATE_FINISHED ==
				    ptask->state) {
			 ...
			 struct adlak_profile_cfg_desc *profile_cfg)
{
	...
	AML_LOG_INFO("profile_iova=0x%lX",
		     (uintptr_t)profile_cfg->profile_iova);
	AML_LOG_INFO("profile_buf_size=%lu KByte",
		     (uintptr_t)(profile_cfg->profile_buf_size / 1024));
...
int adlak_queue_update_task_state(struct adlak_device *padlak,
				  struct adlak_task *ptask)
{
	...
	ASSERT(ptask);
	if (ptask->state != ADLAK_SUBMIT_STATE_FINISHED &&
	...
	}
	{
		...
		if (ptask->flag & ADLAK_TASK_CANCELED) {
			AML_LOG_DEBUG
		("delete the task had canceled which the owner's net_id is %d.",
		 ptask->context->net_id);
			list_del(&ptask->head);
			adlak_invoke_destroy(ptask);
		...
		if (context->state == CONTEXT_STATE_CLOSED)
...
static int adlak_parser_resume(struct adlak_device *padlak,
			       struct adlak_task *ptask,
			       struct adlak_cmq_buffer *cmq_buf_info)
{
	int ret;
	...
	if (!cmq_buf_info->parser_storage_info.size) {
		// parser_storage_info is not initial
		cmq_buf_info->parser_storage_info.base_addr =
			cmq_buf_info->cmq_mm_info->iova_addr;
	...
	if (pmodel_attr->cmq_buffer_type == ADLAK_CMQ_BUFFER_TYPE_PRIVATE) {
		// the cmq is private
		...
		if (ptask->invoke_start_idx == pmodel_attr->hw_layer_first) {
			...
			AML_LOG_DEBUG("read point per-layer[%d] = 0x%08X",
				      ptask->invoke_start_idx,
				      pmodel_attr->cmq_offsets[ptask->invoke_start_idx]
						   .read_point);
				...
				cmq_buf_info->parser_storage_info.rpt =
					pmodel_attr
						->cmq_offsets
							[ptask->invoke_start_idx]
						.read_point;
				cmq_buf_info->parser_storage_info.wpt =
					cmq_buf_info->parser_storage_info.rpt;
				cmq_buf_info->parser_storage_info.ppt =
					cmq_buf_info->parser_storage_info.rpt;
			}
		}
	...
	} else if (ADLAK_CMQ_BUFFER_TYPE_PRIVATE ==
		   pmodel_attr->cmq_buffer_type) {
		// the cmq is private
		if (ptask->invoke_start_idx == pmodel_attr->hw_layer_first)
			resume = 1;
	}
	if (resume != 0) {
		...
		pmodel_attr->cmq_buffer->cmq_rd_offset =
			cmq_buf_info->parser_storage_info.rpt;
		pmodel_attr->cmq_buffer->cmq_wr_offset =
			cmq_buf_info->parser_storage_info.wpt;
	}
			      ...
			      struct adlak_get_stat_desc *stat_desc)
{
	...
	while (1) {
		if (ERR(NONE) ==
		    adlak_os_sema_take_timeout(context->invoke_state,
					       stat_desc->timeout_ms)) {
			find_netid = 0;
			adlak_os_mutex_lock(&pwq->wq_mutex);
			list_for_each_entry_safe(ptask, ptask_tmp,
						 &pwq->finished_list, head) {
				if (ptask &&
				    ptask->context->net_id ==
					    stat_desc->net_register_idx) {
					find_netid = 1;
					if (ptask->invoke_idx ==
					    stat_desc->invoke_register_idx) {
						{
							finished = 1;
#ifdef CONFIG_ADLAK_DEBUG_INNNER
							adlak_dbg_inner_update(context,
									       "poll to umd");
#endif
							break;
						}
	...
	if (finished == 1) {
		...
		stat_desc->profile_en =
			ptask->context->pmodel_attr->pm_cfg.profile_en;
		...
		stat_desc->mem_pool_size = mem_usage.pool_size;
		stat_desc->mem_pool_used = mem_usage.alloced_kmd +
					   mem_usage.alloced_umd +
					   mem_usage.share_buf_size;
...
int adlak_submit_patch_and_exec(struct adlak_task *ptask)
{
	...
	adlak_profile_start(padlak, ptask->context, &pmodel_attr->pm_cfg,
			    &pmodel_attr->pm_stat,
			    (ptask->invoke_start_idx <=
			     ptask->context->pmodel_attr->hw_layer_first) ?
				    1 :
				    0);
...
static int adlak_cmq_public_patch_and_exec(struct adlak_task *ptask)
{
	...
	u8 *config_base = NULL;
	//u32 *                      pcmq_buf                 = NULL;
	s32 pwe_flid_offset, pwx_flid_offset, rs_flid_offset;
	s32 start_id_pwe, start_id_pwx, start_id_rs;
	u32 nop_size;
	u32 wait_cnt;
	struct adlak_cmq_buffer *cmq_buf_info;
	struct adlak_model_attr *pmodel_attr;
	struct adlak_circular_buffer circbuffer_cmq;
	struct adlak_circular_buffer *pcircbuffer;
	s32 config_offset, config_size;
	u64 smmu_entry;
	...
	if (smmu_entry != ADLAK_INVALID_ADDR)
	...
	if (!pmodel_attr->cmd_buf_attr.support)
		config_base = (u8 *)pmodel_attr->config;
	else
		config_base = (u8 *)(uintptr_t)adlak_mem_vmap(pmodel_attr->cmd_buf_attr.mm_info);

	AML_LOG_DEFAULT("tasks_num=%u, ", pmodel_attr->submit_tasks_num);
	AML_LOG_DEFAULT("invoke id[%d-%d], ", ptask->invoke_start_idx,
	...
	pwe_flid_offset =
		(psubmitask_base + ptask->invoke_start_idx)->start_pwe_flid + 1;
	pwx_flid_offset =
		(psubmitask_base + ptask->invoke_start_idx)->start_pwx_flid + 1;
	rs_flid_offset =
		(psubmitask_base + ptask->invoke_start_idx)->start_rs_flid + 1;

	ptask->cmd_offset_start = cmq_buf_info->cmq_wr_offset;

	pcircbuffer->data = adlak_mem_vmap(cmq_buf_info->cmq_mm_info);
	pcircbuffer->size = cmq_buf_info->size / sizeof(u32);
	pcircbuffer->head = cmq_buf_info->cmq_rd_offset / sizeof(u32);
	pcircbuffer->tail = cmq_buf_info->cmq_wr_offset / sizeof(u32);
	...
	for (task_idx = ptask->invoke_start_idx;
	     task_idx <= ptask->invoke_end_idx; task_idx++) {
		psubmitask = psubmitask_base + task_idx;
		...
		// generate commands
		if (config_size % sizeof(u32)) {
			AML_LOG_INFO("The config_size[%d] is not divisible by 4!",
				     config_size);
			ASSERT(0);
		}

		if (adlak_cmq_remain_space_check(padlak, cmq_buf_info,
						 sizeof(u32) * pcircbuffer->tail,
						 ADLAK_ALIGN(sizeof(u32) * 7 + config_size, 16))) {
			ptask->hw_stat.irq_status.timeout = false;
		...
		adlak_cmq_write_data(pcircbuffer, PS_CMD_SET_SW_ID | (task_idx &
						  PS_CMD_SW_ID_MASK)); // cmd_sw_id

		adlak_cmq_write_data(pcircbuffer,
				     adlak_gen_dep_cmd(dependency_mode, psubmitask,
						       psubmit_dep_fixup_base,
						       pwe_flid_offset, pwx_flid_offset,
						       rs_flid_offset, start_id_pwe,
						       start_id_pwx,
						       start_id_rs)); // cmd_dependcy

		adlak_cmq_write_data(pcircbuffer, PS_CMD_EXECUTE |
						  psubmitask->output_modules); // cmd_execute

		adlak_cmq_write_data(pcircbuffer, PS_CMD_CONFIG |
						  psubmitask->active_modules); // cmd_config

		cmq_offset_cfg = adlak_cmq_get_cur_tail(pcircbuffer);
		adlak_cmq_write_buffer(pcircbuffer, (const u32 *)(config_base + config_offset),
				       config_size / sizeof(u32));
		if (pmodel_attr->cmd_buf_attr.support) {
			adlak_cmq_merge_modify_data(psubmitask, pcircbuffer, cmq_offset_cfg,
						    (const u32 *)(config_base +
								  psubmitask->config_v2
									      .modify_offset),
						    psubmitask->config_v2.modify_size);
		}
		adlak_update_addr_fixups(psubmitask, psubmit_addr_fixups_base,
					 pcircbuffer, cmq_offset_cfg);
		adlak_update_module_dependency(dependency_mode, psubmitask,
					       psubmit_dep_fixup_base,
					       pwe_flid_offset, pwx_flid_offset,
					       rs_flid_offset, start_id_pwe,
					       start_id_pwx, start_id_rs,
					       pcircbuffer, cmq_offset_cfg);
		adlak_update_reg_fixups(dependency_mode, psubmitask,
					psubmit_reg_fixup_base, pcircbuffer,
					cmq_offset_cfg);

		adlak_cmq_write_data(pcircbuffer, (PS_CMD_SET_FENCE |
						   psubmitask->fence_modules));
		cmq_size_u32 = 7 + config_size / sizeof(u32);
		nop_size = ADLAK_ALIGN(cmq_size_u32, (16 / sizeof(u32))) -
			   cmq_size_u32;
		...
		if (task_idx == ptask->invoke_end_idx) {
			adlak_cmq_write_data(pcircbuffer,
					     PS_CMD_SET_TIME_STAMP |
						PS_CMD_TIME_STAMP_IRQ_MASK); // cmd_time_stamp
		} else {
			adlak_cmq_write_data(pcircbuffer,
		...
		adlak_flush_cache(padlak, cmq_buf_info->cmq_mm_info,
				  &sync_cache_extern);
		...
		cmq_buf_info->cmq_rd_offset = adlak_hal_get_ps_rpt(padlak);
		wait_cnt = 0;
		while (cmq_buf_info->cmq_wr_offset ==
		       cmq_buf_info->cmq_rd_offset) {
			adlak_os_udelay(10);
			cmq_buf_info->cmq_rd_offset =
				adlak_hal_get_ps_rpt(padlak);
			wait_cnt++;
			if (wait_cnt > 10000) {
		...
		AML_LOG_DEBUG("cmq_wr_offset=%u , cmq_rd_offset=%u",
			      cmq_buf_info->cmq_wr_offset,
			      cmq_buf_info->cmq_rd_offset);
	...
	AML_LOG_INFO("cmd_offset_start=0x%08X cmd_offset_end=0x%08X size=0x%08X",
		     (u32)ptask->cmd_offset_start, (u32)ptask->cmd_offset_end,
		     (u32)(ptask->cmd_offset_end + cmq_buf_info->size - ptask->cmd_offset_start) %
				cmq_buf_info
				->size); // Tips: the size does not account for coverage issues
...
static int adlak_cmq_private_patch_and_exec(struct adlak_task *ptask)
{
	...
	u64 smmu_entry = ADLAK_INVALID_ADDR;
	...
	for (task_idx = ptask->invoke_start_idx;
		...
		adlak_update_addr_fixups(psubmitask,
					 pmodel_attr->submit_addr_fixups,
					 pcircbuffer,
	...
	mb();
	AML_LOG_INFO("cmq_wr_offset = 0x%08X ",
		     (u32)cmq_buf_info->cmq_wr_offset);
	...
	if (smmu_attr->smmu_tlb_type == ADLAK_ENUM_SMMU_TLB_TYPE_PUBLIC_ONLY) {
		...
		smmu_entry = adlak_mem_get_smmu_entry(ptask->context,
						      ADLAK_ENUM_SMMU_TLB_TYPE_PUBLIC_ONLY);
	} else if (smmu_attr->smmu_tlb_type ==
		...
		if (pmodel_attr->hw_layer_last_in_first_smmu_table >=
		    ptask->invoke_end_idx) {
			AML_LOG_INFO
		("case 2: hw_layer_last_in_first_smmu_table >= invoke_end_idx");
			smmu_entry = adlak_mem_get_smmu_entry(ptask->context,
							      ADLAK_ENUM_SMMU_TLB_TYPE_PUBLIC_ONLY);
		} else if (pmodel_attr->hw_layer_last_in_first_smmu_table <
			   ptask->invoke_start_idx) {
			AML_LOG_INFO
		("case 3: hw_layer_last_in_first_smmu_table < invoke_start_idx");
			smmu_entry = adlak_mem_get_smmu_entry
		(ptask->context, ADLAK_ENUM_SMMU_TLB_TYPE_PRIVATE_ONLY);
		} else {
			AML_LOG_INFO
		("case 4: invoke_start_idx < hw_layer_last_in_first_smmu_table < invoke_end_idx");
			cmq_buf_info->cmq_wr_offset =
				pmodel_attr
					->cmq_offsets
						[pmodel_attr
							 ->hw_layer_last_in_first_smmu_table]
					.write_point;
			smmu_entry = adlak_mem_get_smmu_entry(ptask->context,
							      ADLAK_ENUM_SMMU_TLB_TYPE_PUBLIC_ONLY);
			if (smmu_entry != ADLAK_INVALID_ADDR) {
			...
			cmq_buf_info->cmq_wr_offset =
				pmodel_attr->cmq_offsets[ptask->invoke_end_idx]
					.write_point;
			smmu_entry =
				adlak_mem_get_smmu_entry(ptask->context,
							 ADLAK_ENUM_SMMU_TLB_TYPE_PRIVATE_ONLY);
		}
	...
	if (smmu_entry != ADLAK_INVALID_ADDR) { // update smmu table
		adlak_hal_set_mmu(padlak, true, smmu_entry);
	}
	adlak_hal_submit((void *)padlak, cmq_buf_info->cmq_wr_offset);
#if CONFIG_ADLAK_EMU_EN
...
static int adlak_cmq_private_fill(struct adlak_model_attr *pmodel_attr)
{
	...
	struct {
		u32 id;
		u32 timestamp_offset; // the offset in the command queue buffer
	} hw_node_pre = { 0 };
	...
	pwe_flid_offset =
		(psubmitask_base + pmodel_attr->hw_layer_first)->start_pwe_flid +
		1;
	pwx_flid_offset =
		(psubmitask_base + pmodel_attr->hw_layer_first)->start_pwx_flid +
		1;
	rs_flid_offset =
		(psubmitask_base + pmodel_attr->hw_layer_first)->start_rs_flid +
		1;
	...
	pmodel_attr->cmq_offsets =
		(struct adla_context_cmq_offset *)
		adlak_os_zalloc(sizeof(u32) * pmodel_attr->submit_tasks_num * 3,
				ADLAK_GFP_KERNEL);

	for (task_idx = 0; task_idx <= pmodel_attr->hw_layer_last; task_idx++) {
		cmq_offset = &pmodel_attr->cmq_offsets[task_idx];

		cmq_offset->read_point =
			adlak_cmq_get_cur_tail(pcircbuffer) * sizeof(u32);
		AML_LOG_DEBUG("set read point per layer[%d] = 0x%08X", task_idx,
			      cmq_offset->read_point);
...
// software operations
#ifndef CONFIG_ADLAK_FIXUP_TIMESTAMP_ENABLE
			if (task_idx != pmodel_attr->hw_layer_first &&
			    hw_node_pre.timestamp_offset != 0) {
				// insert irq flag in the command of last node
				AML_LOG_INFO("Insert timestamp irq flag for node %u\n",
					     hw_node_pre.id);
				adla_cmq_modify_data(pcircbuffer,
						     hw_node_pre.timestamp_offset,
						     PS_CMD_SET_TIME_STAMP |
						     PS_CMD_TIME_STAMP_IRQ_MASK);
			}
#endif
			continue;
		}
		...
		adlak_cmq_write_data(pcircbuffer,
				     adlak_gen_dep_cmd(dependency_mode, psubmitask,
						       ...
						       pwe_flid_offset, pwx_flid_offset,
						       rs_flid_offset, start_id_pwe,
						       start_id_pwx, start_id_rs)); // cmd_dependcy

		// cmd_execute
		adlak_cmq_write_data(pcircbuffer, PS_CMD_EXECUTE | psubmitask->output_modules);

		// cmd_config
		adlak_cmq_write_data(pcircbuffer, PS_CMD_CONFIG | psubmitask->active_modules);
		...
		adlak_cmq_write_buffer(pcircbuffer, (const u32 *)(config_base + config_offset),
				       config_size / sizeof(u32));
		if (pmodel_attr->cmd_buf_attr.support) {
			adlak_cmq_merge_modify_data(psubmitask, pcircbuffer,
						    cmq_offset->config_offset,
						    (const u32 *)(config_base +
								  psubmitask->config_v2
								  .modify_offset),
		...
		adlak_update_module_dependency(dependency_mode, psubmitask, psubmit_dep_fixup_base,
					       pwe_flid_offset, pwx_flid_offset, rs_flid_offset,
					       start_id_pwe, start_id_pwx, start_id_rs,
					       pcircbuffer, cmq_offset->config_offset);
		adlak_update_reg_fixups(dependency_mode, psubmitask,
					psubmit_reg_fixup_base, pcircbuffer,
					cmq_offset->config_offset);
		if (pmodel_attr->hw_layer_last == task_idx ||
			...
			adlak_cmq_write_data(pcircbuffer,
					     (PS_CMD_SET_FENCE |
					      (PS_CMD_FENCE_PWX_MASK |
					       PS_CMD_FENCE_PWE_MASK |
		...
		hw_node_pre.timestamp_offset =
			adlak_cmq_get_cur_tail(pcircbuffer);
		if (pmodel_attr->hw_layer_last == task_idx ||
		    (pmodel_attr->context->smmu_attr.smmu_tlb_type ==
			     ADLAK_ENUM_SMMU_TLB_TYPE_PRIVATE_AND_PUBLIC &&
		     pmodel_attr->hw_layer_last_in_first_smmu_table ==
			...
			adlak_cmq_write_data(pcircbuffer, PS_CMD_SET_TIME_STAMP); // cmd_time_stamp
		}
#else
		...
		cmq_offset->write_point =
			adlak_cmq_get_cur_tail(pcircbuffer) * sizeof(u32);
		AML_LOG_DEBUG("set write point per layer[%d] = 0x%08X",
			      task_idx, cmq_offset->write_point);
	}
...
#endif
	pmodel_attr->cmq_buffer_type = ADLAK_CMQ_BUFFER_TYPE_PUBLIC;
	if (psubmit_desc->cmq_buffer_type == ADLAK_CMQ_BUFFER_TYPE_PUBLIC)
		goto err;

	cmq_buf_info = adlak_os_zalloc(sizeof(struct adlak_cmq_buffer),
				       ADLAK_GFP_KERNEL);
	if (!cmq_buf_info)
		goto err;
